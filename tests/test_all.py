...
page = c.get("/banks").get_data(as_text=True)
cat = importers.catalogue()
check("the Banks & formats page lists every reader by name, sorted, one line per bank, with its papers and kinds",
...
check("...and asks for a redacted sample of a bank that is missing, through the issue form",
      "issues/new?template=new_reader.yml" in page, True)
check("...it is in the navigation and linked from the import page",
      ("/banks" in c.get("/accounts").get_data(as_text=True), "/banks" in c.get(f"/accounts/{depot_id}/import").get_data(as_text=True)), (True, True))
...
with db.get_conn() as conn:
    ...
    # the same buy as the CSV export brought it years ago — the fee beside the money
    # rather than inside it, the units to six places — and the same deposit
    conn.execute("INSERT INTO transactions (account_id, txn_date, description, amount, currency, external_id, kind, isin, security_name, quantity, price, fee, source) "
                 "VALUES (?, '2026-03-03', 'Kauf Vanguard FTSE All-World', -315.5, 'EUR', 'tr:csv-1', 'buy', 'IE00BK5BQT80', 'Vanguard FTSE All-World', 3.000001, 105.5, 1.0, 'trade_republic')", (tr_new_id,))
...
check("a sync into an account that holds the history adds only what is new — the fee inside or beside the total, the day give or take one — and deletes its own earlier double",
