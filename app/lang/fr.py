...
STRINGS: dict[str, str] = {
    ...
    "Rules": "Règles",
    "Re-apply every rule": "Réappliquer toutes les règles",
    "Every rule re-applied: {n} transaction filed.": "Toutes les règles réappliquées : {n} transaction classée.",
    "Every rule re-applied: {n} transactions filed.": "Toutes les règles réappliquées : {n} transactions classées.",
    "not a subscription": "pas un abonnement",
    "it is a subscription": "c’est un abonnement",
    "put it back": "la remettre",
    "Dismissed": "Écartés",
    "you said these are not subscriptions; they stay out of the totals": "vous avez dit que ce ne sont pas des abonnements ; ils restent hors des totaux",
    ...
    # ─── Upcoming ───
    'Upcoming': 'À venir',
    "Today's cash, carried forward through what is known to be coming: the bills as declared, the subscriptions the app has found, the salary as the newest payslip had it. Where the balance gets lowest, and the day it would cross zero — before it does, not after.": "Le liquide d'aujourd'hui, projeté à travers ce que l'on sait venir : les factures telles que déclarées, les abonnements que l'app a repérés, le salaire tel que la dernière fiche de paie le donnait. Où le solde descend le plus bas, et le jour où il passerait sous zéro — avant, pas après.",
    'Cash today': "Liquide aujourd'hui",
    '{n} cash account': '{n} compte courant',
    '{n} cash accounts': '{n} comptes courants',
    'newest reading each': 'le dernier relevé de chacun',
    'In {n} days': 'Dans {n} jours',
    '{inflow} in, {outflow} out': '{inflow} entrent, {outflow} sortent',
    'Lowest point': 'Point le plus bas',
    'below zero on {date}, after {name}': 'sous zéro le {date}, après {name}',
    'on {date}, after {name}': 'le {date}, après {name}',
    'starts below zero — an overdraft or a card, not a crossing': 'commence sous zéro — un découvert ou une carte, pas un passage',
    'nothing scheduled': 'rien de prévu',
    'Scheduled': 'Prévu',
    '{bills} bills, {subs} subscriptions, {income} pay days': '{bills} factures, {subs} abonnements, {income} jours de paie',
    'No exchange rate for {currencies}; what is in them is left out of the figures above.': 'Pas de taux de change pour {currencies} ; ce qui est dans ces devises manque aux chiffres ci-dessus.',
    'Balance ahead': 'Solde à venir',
    '{n} days': '{n} jours',
    'Nothing is scheduled in this window. Declare a bill, or import a payslip, and the line starts moving.': 'Rien de prévu sur cette période. Déclare une facture, ou importe une fiche de paie, et la ligne se met à bouger.',
    'Bills →': 'Factures →',
    'Day by day': 'Jour par jour',
    'income lands before bills on the same day · ~ an amount carried from what it last cost': 'les revenus arrivent avant les factures le même jour · ~ un montant repris de ce que cela a coûté la dernière fois',
    'bill': 'facture',
    'subscription': 'abonnement',
    'missed — expected today': "manquée — attendue aujourd'hui",
    'Starting from': 'Point de départ',
    'No cash account yet.': 'Pas encore de compte courant.',
    'Loan instalments count only when declared as a bill: the app knows what a loan costs, not which account pays it.': "Les échéances de prêt ne comptent que déclarées comme facture : l'app sait ce que coûte un prêt, pas quel compte le paie.",
    ...
    # ─── The loan's own page ─────────────────────────────────────────
    'A loan on its schedule: {balance} still owed, paid off {date}.': 'Un prêt sur son échéancier : {balance} encore dus, soldé le {date}.',
    "A {currency} loan on its fixed {rhythm} schedule from the first instalment of {first} until it is paid off. Every figure is computed from the terms — a constant annuity, interest rounded to the cent each period, the way a bank does it — and reproduces the bank's own breakdown.": "Un prêt en {currency} sur son échéancier fixe ({rhythm}), de la première échéance du {first} jusqu'au remboursement complet. Chaque chiffre est calculé à partir des conditions — une annuité constante, les intérêts arrondis au centime à chaque période, comme le fait une banque — et reproduit le décompte de la banque.",
    'Amortisation schedule': "Tableau d'amortissement",
    'At this rate the loan is overwhelmingly principal from day one — the interest bars are barely visible, which is exactly why the balance falls almost linearly.': "À ce taux, le prêt est presque uniquement du capital dès le premier jour — les barres d'intérêts se voient à peine, et c'est exactement pourquoi le solde baisse de façon quasi linéaire.",
    'Drawn as': 'Débloqué en',
    'Every payment is a fixed annuity: interest, shrinking, against capital repaid.': 'Chaque versement est une annuité fixe : des intérêts, décroissants, face au capital remboursé.',
    'From {amount} at drawdown to zero on {date}.': 'De {amount} au déblocage à zéro le {date}.',
    'Original loan ({year})': 'Prêt initial ({year})',
    'Outstanding': 'Restant dû',
    'Outstanding balance over time': 'Capital restant dû dans le temps',
    "From the reading of {date} the schedule runs from {amount}, the lender's figure, not the sum's — a rate change, a fee, a rounding rule all live in that number. Record a newer balance on the account and it takes over from there.": "À partir du relevé du {date}, l'échéancier repart de {amount}, le chiffre du prêteur et non celui du calcul — un changement de taux, des frais, une règle d'arrondi tiennent tous dans ce seul nombre. Saisis un solde plus récent sur le compte et il prend le relais à partir de là.",
    'from the reading of {date}': 'depuis le relevé du {date}',
    'From here the schedule runs from the reading of {date}.': "À partir d'ici, l'échéancier repart du relevé du {date}.",
    'reading': 'relevé',
    'Outstanding today': "Restant dû aujourd'hui",
    'Payment': 'Versement',
    'Payoff date': 'Date de fin',
    'Repaid so far': 'Remboursé à ce jour',
    'Show amounts in:': 'Afficher les montants en :',
    'That account already has its terms.': 'Ce compte a déjà ses conditions.',
    'That account is not a loan.': "Ce compte n'est pas un prêt.",
    'The dashed markers show today and the final payoff. The balance steps down on each payment date; because the interest share shrinks as the balance falls, the curve gently accelerates toward zero.': "Les repères en pointillés marquent aujourd'hui et la dernière échéance. Le solde descend d'un cran à chaque date de paiement ; comme la part d'intérêts diminue avec le solde, la courbe accélère doucement vers zéro.",
    "The loan is in {currency}; {base} is shown for reference only, at today's rate.": "Le prêt est en {currency} ; {base} n'est indiqué qu'à titre de référence, au taux du jour.",
    'The loan over time →': 'Le prêt dans le temps →',
    'The terms are on record: the balance follows the schedule from here, and its history runs from the first instalment.': "Les conditions sont enregistrées : le solde suit désormais l'échéancier, et son historique part de la première échéance.",
    'This is a loan. Give it its terms': "C'est un prêt. Donnez-lui ses conditions",
    'Today': "Aujourd'hui",
    'Total interest over the whole life of the loan: {amount}.': 'Total des intérêts sur toute la durée du prêt : {amount}.',
    'Where each instalment goes': 'Où va chaque échéance',
    'and its balance follows the schedule, with the page a mortgage deserves': "et son solde suivra l'échéancier, avec la page qu'un prêt immobilier mérite",
    "at today's rate of {rate} {base} per {currency}": 'au taux du jour de {rate} {base} pour 1 {currency}',
    'change the terms': 'modifier les conditions',
    'faded rows are still in the future': 'les lignes estompées sont encore à venir',
    'the loan is denominated in {currency}': 'le prêt est libellé en {currency}',
    '{amount} @ {rate} on drawdown': '{amount} @ {rate} au déblocage',
    '{currency} (loan currency)': '{currency} (devise du prêt)',
    "{currency} (today's rate)": '{currency} (taux du jour)',
    '{name} — loan over time': '{name} — le prêt dans le temps',
    '{n} instalment': '{n} échéance',
    '{n} instalments': '{n} échéances',
    '{n} left': '{n} restantes',
    '{pct} % of the original': '{pct} % du montant initial',
    '{rate} % per year': '{rate} % par an',
    'The instalment is the figure on the contract; the rate the nominal one per year. “Drawn as” is what the loan was worth in another currency the day it began — a CHF mortgage taken out as €150 000 — and stays on the page as the reference it is.': "L'échéance est le montant du contrat ; le taux, le taux nominal annuel. « Débloqué en » est ce que valait le prêt dans une autre devise le jour où il a commencé — un prêt en CHF contracté pour 150 000 € — et reste sur la page comme la référence qu'il est.",

    # ─── Income: payslips ────────────────────────────────────────────
    "A payslip is imported into the account the net salary lands in: open that account, Import a file, choose the PDF — several at once is fine. The sheet is kept whole here, and what the bank never sees is booked onto the account, zero-sum: the tax at source and each side's pension contribution, grossed up and then paid or invested. The net itself is left to the bank.": "Une fiche de paie s'importe dans le compte où arrive le salaire net : ouvrez ce compte, Importer un fichier, choisissez le PDF — plusieurs à la fois, c'est possible. La fiche est conservée entière ici, et ce que la banque ne voit jamais est comptabilisé sur le compte, à somme nulle : l'impôt à la source et la cotisation de retraite de chaque côté, en brut puis payés ou investis. Le net lui-même est laissé à la banque.",
    'Adding a payslip': 'Ajouter une fiche de paie',
    'Bonus': 'Prime',
    'Employer pension': 'Retraite employeur',
    "Employer's cost": 'Coût employeur',
    'Gross': 'Brut',
    'Month': 'Mois',
    'Net paid': 'Net versé',
    'Payslip removed, with the rows it had booked.': "Fiche de paie supprimée, avec les lignes qu'elle avait comptabilisées.",
    'Pension, both sides': 'Retraite, les deux côtés',
    'Remove this payslip and the rows it booked?': "Supprimer cette fiche de paie et les lignes qu'elle a comptabilisées ?",
    'Social': 'Charges sociales',
    'Tax at source': 'Impôt à la source',
    'That payslip is not there.': "Cette fiche de paie n'existe pas.",
    "The household's payslips: gross pay, what was taken at source, the pension on both sides, the bonuses — per earner. The bank saw the net arrive; the sheet saw the rest, and that is what a tax return and a retirement forecast ask for.": "Les fiches de paie du foyer : le brut, ce qui a été prélevé à la source, la retraite des deux côtés, les primes — par salarié. La banque a vu arriver le net ; la fiche a vu le reste, et c'est ce que demandent une déclaration d'impôts et une projection de retraite.",
    "Two layouts are read: the SAP Lohnabrechnung of a large employer, with wage codes, and the small employer's sheet with Bruttolohn, AHV, ALV, BVG and Nettolohn. The earner's name is taken from the sheet, so one household's payslips sort themselves.": "Deux mises en page sont lues : la Lohnabrechnung SAP d'un grand employeur, avec ses codes de paie, et la fiche du petit employeur avec Bruttolohn, AHV, ALV, BVG et Nettolohn. Le nom du salarié est pris sur la fiche, si bien que les fiches d'un foyer se classent d'elles-mêmes.",
    'Your accounts a salary could land in:': 'Vos comptes où un salaire peut arriver :',
    'bonus {amount}': 'prime {amount}',
    "gross plus the employer's contributions": "le brut plus les cotisations de l'employeur",
    'gross {amount}': 'brut {amount}',
    'of {gross} gross': 'sur {gross} brut',
    'paid {date}': 'versé le {date}',
    'pension inflow {amount}': 'retraite alimentée de {amount}',
    'tax {pct} %': 'impôt {pct} %',
    'the statutory floor — the sheet prints no employer block': "le plancher légal — la fiche n'imprime pas de bloc employeur",
    "{mine} yours, {theirs} the employer's": "{mine} de vous, {theirs} de l'employeur",
    '{n} month': '{n} mois',
    '{n} months': '{n} mois',
    '{pct} % of gross': '{pct} % du brut',

    # ─── The payslip mapper ──────────────────────────────────────────
    'A sheet naming one of these employers and earners is read through its mapping without asking. Forget one and the next such sheet asks again — the payslips already imported stay.': "Une fiche nommant l'un de ces employeurs et salariés est lue à travers sa correspondance sans rien demander. Oubliez-en une et la prochaine fiche de ce type redemande — les fiches déjà importées restent.",
    'Allowance': 'Indemnité',
    'Amounts': 'Montants',
    'Base salary': 'Salaire de base',
    "Both names must appear on the sheet as written here — spaces and case do not matter — because that is how the next sheet is recognised. The earner's name is what the Income page groups by.": "Les deux noms doivent figurer sur la fiche tels qu'écrits ici — espaces et casse sans importance — car c'est ainsi que la prochaine fiche est reconnue. Le nom du salarié est ce par quoi la page Revenus regroupe.",
    'Earner': 'Salarié',
    'Employer': 'Employeur',
    "Employer's side": 'Côté employeur',
    'Label': 'Libellé',
    'Lines': 'Lignes',
    'Map the payslip': 'Faire correspondre la fiche de paie',
    'Mapping forgotten. The next sheet from that employer asks again.': 'Correspondance oubliée. La prochaine fiche de cet employeur redemande.',
    'Means': 'Signifie',
    "No importer here knows this sheet, and it does not need to: say which line is which, once. The mapping is kept under the sheet's own markers — the employer and the earner — so next month's sheet from the same employer is recognised by itself. The suggestions come from a catalogue of what payslips call things in the languages they are printed in; what is remembered is what you confirm.": "Aucun importateur ici ne connaît cette fiche, et il n'en a pas besoin : dites une fois quelle ligne est quoi. La correspondance est conservée sous les repères propres de la fiche — l'employeur et le salarié — si bien que la fiche du mois suivant du même employeur est reconnue d'elle-même. Les suggestions viennent d'un catalogue de ce que les fiches de paie appellent les choses, dans les langues où elles sont imprimées ; ce qui est retenu est ce que vous confirmez.",
    'On a line with several amounts the last one is the amount; the ones before it are the basis and the rate. Deductions are stored as money out whichever way the sheet prints them — a French bulletin prints its retenues positive, an SAP sheet with a trailing minus — because what a line means settles the sign. Several lines may mean the same thing: AHV and ALV are both social deductions. Lines left at — are kept as detail and count for nothing.': "Sur une ligne à plusieurs montants, le dernier est le montant ; ceux d'avant sont la base et le taux. Les retenues sont enregistrées en sortie d'argent quelle que soit la façon dont la fiche les imprime — un bulletin français imprime ses retenues en positif, une fiche SAP avec un moins final — parce que ce qu'une ligne signifie décide du signe. Plusieurs lignes peuvent signifier la même chose : AHV et ALV sont toutes deux des charges sociales. Les lignes laissées à — sont gardées comme détail et ne comptent pour rien.",
    'Other deduction': 'Autre retenue',
    'Paid on': 'Versé le',
    'Payslip mappings': 'Correspondances de fiches de paie',
    "Pension, the employer's": 'Retraite, part employeur',
    'Pension, yours': 'Retraite, votre part',
    'Rate': 'Taux',
    'Read it through this mapping': 'Lire à travers cette correspondance',
    'Read through the mapping': 'Lu à travers la correspondance',
    'Social deduction': 'Charge sociale',
    "Social, the employer's": 'Charge sociale, part employeur',
    'Taken at source': 'Prélevé à la source',
    'The gross and the net paid, the employer and the earner — that is the least a payslip mapping needs.': "Le brut et le net versé, l'employeur et le salarié — c'est le minimum qu'une correspondance de fiche de paie exige.",
    'The lines': 'Les lignes',
    'The mapping is saved as {name}; the next sheet from this employer is recognised by itself.': "La correspondance est enregistrée sous {name} ; la prochaine fiche de cet employeur est reconnue d'elle-même.",
    'The month': 'Le mois',
    'This is not a payslip': "Ce n'est pas une fiche de paie",
    'Whose sheet': 'La fiche de qui',
    'amounts written the {fmt} way': 'montants écrits comme {fmt}',
    'base {amount}': 'base {amount}',
    'gross less every deduction is the net: the mapping caught every line': 'le brut moins chaque retenue donne le net : la correspondance a saisi chaque ligne',
    'gross less the deductions differs from the net by {gap} — a line is missing or mapped twice': 'le brut moins les retenues diffère du net de {gap} — une ligne manque ou est comptée deux fois',
    "nothing mapped as the employer's — the sheet may print none": "rien d'attribué au côté employeur — la fiche n'en imprime peut-être pas",
    'pension {pension} · social {social}': 'retraite {pension} · social {social}',
    'tax {tax} · social {social} · pension {pension} · other {other}': 'impôt {tax} · social {social} · retraite {pension} · autre {other}',
    '{n} line with an amount, a month or a date': '{n} ligne avec un montant, un mois ou une date',
    ...
    'Your bank is not here?': "Votre banque n'y est pas ?",
    'Every reader was written from one sample statement, and yours can be the next. Send a redacted statement — name, address, account number and IBAN painted over; dates, amounts, ISINs and every printed label kept — and it becomes a reader in the next release. A PDF with the boxes blacked out is fine, and so is its text layer copied out of a viewer. One document per kind helps most: a purchase, a dividend, a Kontoauszug.': "Chaque lecteur est né d'un relevé d'exemple, et le vôtre peut être le prochain. Envoyez un relevé anonymisé — nom, adresse, numéro de compte et IBAN masqués ; dates, montants, ISIN et chaque libellé imprimé conservés — et il devient un lecteur dans la prochaine version. Un PDF aux cases noircies suffit, comme sa couche texte copiée depuis une visionneuse. Un document par type aide le plus : un achat, un dividende, un relevé.",
    'Send a statement sample': "Envoyer un relevé d'exemple",
    'opens a GitHub issue with a short form and a place to attach the file': 'ouvre un ticket GitHub avec un court formulaire et un espace pour joindre le fichier',
    'The same goes for a bank that is listed but reads a figure wrong: the sample that shows it is the fix.': "Idem pour une banque listée dont un chiffre est mal lu : l'exemple qui le montre est le correctif.",
    '{n} banks, brokers and formats. Type a name to find yours; a bank not here is read through a CSV mapping you draw once — or send a statement, above.': '{n} banques, courtiers et formats. Tapez un nom pour trouver le vôtre ; une banque absente se lit par un mappage CSV dessiné une fois — ou envoyez un relevé, ci-dessus.',
    'Nothing by that name — send a statement sample, above, or map a CSV of it once on the import page.': "Rien sous ce nom — envoyez un relevé d'exemple, ci-dessus, ou mappez un CSV une fois sur la page d'import.",
    'API secret': "Secret d'API",
    'Add your Interactive Brokers Flex token under Settings first.': "Ajoutez d'abord votre jeton Flex Interactive Brokers dans les Réglages.",
    'Add your Trading 212 API key under Settings first.': "Ajoutez d'abord votre clé API Trading 212 dans les Réglages.",
    'Approve the login in the Trade Republic app, then:': "Approuvez la connexion dans l'app Trade Republic, puis :",
    'Code from the Trade Republic app': "Code de l'app Trade Republic",
    'Connect Interactive Brokers': 'Connecter Interactive Brokers',
    'Connect Trade Republic': 'Connecter Trade Republic',
    'Connect Trading 212': 'Connecter Trading 212',
    'Finish the login': 'Terminer la connexion',
    'Flex Query id': 'Id de la Flex Query',
    'Flex Web Service token': 'Jeton Flex Web Service',
    'Forget Trade Republic': 'Oublier Trade Republic',
    'Forget the Interactive Brokers token': 'Oublier le jeton Interactive Brokers',
    'Forget the Trading 212 key': 'Oublier la clé Trading 212',
    'In the Trading 212 app: Settings → API (Beta) → Generate key. Give it only the read scopes — account data, portfolio, history — and paste the key and the secret here; the secret is shown once. Invest and Stocks ISA accounts only. The API allows six history calls a minute, so a first sync of years of orders takes a few minutes; later syncs are quick.': "Dans l'app Trading 212 : Réglages → API (bêta) → Générer une clé. Ne lui donnez que les droits de lecture — données du compte, portefeuille, historique — et collez ici la clé et le secret ; le secret n'est montré qu'une fois. Comptes Invest et Stocks ISA uniquement. L'API autorise six appels d'historique par minute : une première synchronisation d'années d'ordres prend quelques minutes ; les suivantes sont rapides.",
    'Interactive Brokers answers: account {accounts}, statement {start} to {end}, {trades} trades. Now connect an account from its page.': 'Interactive Brokers répond : compte {accounts}, relevé du {start} au {end}, {trades} transactions. Connectez maintenant un compte depuis sa page.',
    'Interactive Brokers token forgotten. The account and its history stay.': 'Jeton Interactive Brokers oublié. Le compte et son historique restent.',
    'Interactive Brokers: link this account to the Flex query under Settings and pull every trade, dividend and cash movement it covers.': "Interactive Brokers : liez ce compte à la Flex Query des Réglages et récupérez chaque transaction, dividende et mouvement d'espèces qu'elle couvre.",
    'Log in to Trade Republic': 'Se connecter à Trade Republic',
    'Log in to Trade Republic first.': "Connectez-vous d'abord à Trade Republic.",
    'Logged in to Trade Republic, securities account {account}.': 'Connecté à Trade Republic, compte-titres {account}.',
    "No login and no app: IBKR's Flex Web Service. In Account Management → Reports → Flex Queries, make an Activity Flex Query with Trades, Cash Transactions, Open Positions and Cash Report ticked, period Last 365 Days, XML; note its id. Under Reports → Settings → Flex Web Service, switch it on and copy the token. The token can only run Flex queries — it cannot see your login, trade or move money. For what lies before the year the query covers, run it once for a custom period and drop the XML on the import page.": "Ni connexion ni app : le Flex Web Service d'IBKR. Dans Account Management → Reports → Flex Queries, créez une Activity Flex Query avec Trades, Cash Transactions, Open Positions et Cash Report cochés, période Last 365 Days, XML ; notez son id. Sous Reports → Settings → Flex Web Service, activez le service et copiez le jeton. Le jeton ne peut qu'exécuter des Flex Queries — il ne voit pas votre identifiant, ne passe pas d'ordre et ne déplace pas d'argent. Pour ce qui précède l'année couverte, exécutez-la une fois sur une période personnalisée et déposez le XML sur la page d'import.",
    'Not approved yet — approve the login in the Trade Republic app, then press Finish again.': "Pas encore approuvé — approuvez la connexion dans l'app Trade Republic, puis appuyez de nouveau sur Terminer.",
    'PIN': 'PIN',
    'Phone number': 'Numéro de téléphone',
    'Save and run the query once': 'Enregistrer et exécuter la requête une fois',
    'The Trade Republic login has lapsed — log in again to resume syncing.': 'La connexion Trade Republic a expiré — reconnectez-vous pour reprendre la synchronisation.',
    'Trade Republic forgotten — phone, PIN and login. The account and its history stay.': 'Trade Republic oublié — téléphone, PIN et connexion. Le compte et son historique restent.',
    'Trade Republic is asking: approve the login in the app, then press Finish.': "Trade Republic demande : approuvez la connexion dans l'app, puis appuyez sur Terminer.",
    "Trade Republic publishes no API. This uses the interface its own web app uses — phone number, PIN, the app's approval, then the same WebSocket — and it may stop working any day, as it has before; the statement PDFs always read. The phone number and PIN are kept 0600 beside the other keys; a login lasts until Trade Republic ends it, and the account page says when it has to be renewed.": "Trade Republic ne publie aucune API. Ceci utilise l'interface de sa propre app web — numéro de téléphone, PIN, approbation dans l'app, puis le même WebSocket — et peut cesser de fonctionner du jour au lendemain, comme déjà arrivé ; les PDF de relevés se lisent toujours. Le téléphone et le PIN sont gardés 0600 à côté des autres clés ; une connexion dure jusqu'à ce que Trade Republic y mette fin, et la page du compte indique quand la renouveler.",
    'Trade Republic wants the code from the app: type it and press Finish.': "Trade Republic veut le code de l'app : saisissez-le et appuyez sur Terminer.",
    'Trade Republic, through the interface its web app uses — unofficial, and it may stop working any day; the statement PDFs always read.': "Trade Republic via l'interface de son app web — non officielle, elle peut cesser de fonctionner à tout moment ; les PDF de relevés se lisent toujours.",
    'Trading 212 key forgotten. The account and its history stay.': 'Clé Trading 212 oubliée. Le compte et son historique restent.',
    'Trading 212 key works: {cash} {currency} free cash, {invested} invested. Now connect an account from its page.': 'La clé Trading 212 fonctionne : {cash} {currency} de liquidités, {invested} investis. Connectez maintenant un compte depuis sa page.',
    'Trading 212: link this account to the API key under Settings and pull every order, dividend and cash movement. A long history takes a few minutes — the API allows six calls a minute.': "Trading 212 : liez ce compte à la clé API des Réglages et récupérez chaque ordre, dividende et mouvement d'espèces. Un long historique prend quelques minutes — l'API autorise six appels par minute.",
    'Unofficial interface; the login is kept until Trade Republic ends it. Syncs with the daily sync.': "Interface non officielle ; la connexion est gardée jusqu'à ce que Trade Republic y mette fin. Synchronisée avec la synchronisation quotidienne.",
    'live': 'réel',
    'logged in': 'connecté',
    'not logged in': 'non connecté',
    'practice': 'entraînement',
    'practice account': "compte d'entraînement",
    'token saved': 'jeton enregistré',
    'unofficial': 'non officiel',
    'Phone number and PIN': 'Numéro de téléphone et PIN',
    'saved': 'enregistré',
    'Log in': 'Se connecter',
    'Trade Republic will ask its app to approve, or for a code.': "Trade Republic demandera l'approbation dans son app, ou un code.",
    'After step 1.': "Après l'étape 1.",
    'After step 2.': "Après l'étape 2.",
    'Connect to an account': 'Connecter à un compte',
    'a new account called Trade Republic': 'un nouveau compte nommé Trade Republic',
    'Connect and sync': 'Connecter et synchroniser',
    'Logged in to Trade Republic, securities account {account}. Now pick the account below to connect.': 'Connecté à Trade Republic, compte-titres {account}. Choisissez maintenant ci-dessous le compte à connecter.',
    'Saved. Now step 2: log in.': "Enregistré. Maintenant l'étape 2 : se connecter.",
    '1 day': '1 jour',
    '1 week': '1 semaine',
    '3 months': '3 mois',
    '3 years': '3 ans',
    'Since start': 'Depuis le début',
    'from {date}: time-weighted return, and the gain beyond the money moved in the meantime': "depuis {date} : rendement pondéré dans le temps, et le gain au-delà de l'argent déplacé entre-temps",
    'nothing to measure yet': 'rien à mesurer encore',
    'Performance': 'Performance',
    'Each period: the time-weighted return, and underneath the gain in money — what the value did beyond the deposits and withdrawals of the period, so a purchase is not a gain and a sale not a loss. A period the records do not reach back to is measured from the first day they do.': "Par période : le rendement pondéré dans le temps et, dessous, le gain en argent — ce que la valeur a fait au-delà des dépôts et retraits de la période, si bien qu'un achat n'est pas un gain ni une vente une perte. Une période que les enregistrements n'atteignent pas est mesurée depuis leur premier jour.",
    'time-weighted and money-weighted, side by side': "pondéré dans le temps et par l'argent, côte à côte",
    'Portfolio performance': 'Performance du portefeuille',
    'the securities, time-weighted — cash left out': 'les titres, pondérés dans le temps — sans les liquidités',
    '1 month': '1 mois',
    '1 year': '1 an',
    'A rule has to do something: file under a category, rename the counterparty, set the kind, add a tag, or say whose spending it is.': 'Une règle doit faire quelque chose : classer sous une catégorie, renommer la contrepartie, fixer le type, ajouter une étiquette ou dire à qui est la dépense.',
    'Doubled bank rows removed': 'Lignes bancaires en double supprimées',
    'Its category_rules.json, if you have one': "Son category_rules.json, si vous l'avez",
    'No spending in this window.': 'Aucune dépense sur cette période.',
    'Nobody yet': 'Personne encore',
    'People created': 'Personnes créées',
    'Rules brought along': 'Règles reprises',
    'Shared': 'Partagé',
    "The household's spending split between its people. A row of spending belongs to one of you, or to the household — shared, halved — or to nobody yet; what nobody has claimed sits in its own bucket rather than being quietly absorbed. Say whose a row is on the Transactions page, and it becomes a rule for the next one like it.": "Les dépenses du ménage réparties entre ses personnes. Une dépense appartient à l'un de vous, au ménage — partagée, divisée en deux — ou à personne encore ; ce que personne n'a réclamé reste dans sa propre case plutôt que d'être absorbé en silence. Dites à qui est une ligne sur la page Transactions, et cela devient une règle pour la suivante du même genre.",
    'The month before, {month}: {total} in all': "Le mois d'avant, {month} : {total} en tout",
    'Who spent': 'Qui a dépensé',
    'Whose spending': 'Dépense de qui',
    "each person's own spending plus half of what was shared": 'les dépenses propres de chacun plus la moitié du partagé',
    'everything is assigned': 'tout est attribué',
    'last 12 months': '12 derniers mois',
    'month still running': 'mois en cours',
    'shared': 'partagé',
    'split evenly': 'à parts égales',
    'whose spending': 'dépense de qui',
    ...
    "An app on your phone asks for this dashboard's address and a six-digit code. The code is good for five minutes and for one device; it is exchanged once for the token above, over the same address you are reading this on. A token shown on a screen is a token anyone who photographs the screen owns, which is why the screen shows a code instead.": "Une app sur votre téléphone demande l'adresse de ce tableau de bord et un code à six chiffres. Le code vaut cinq minutes et pour un appareil ; il s'échange une fois contre le jeton ci-dessus, par l'adresse même où vous lisez ceci. Un jeton affiché à l'écran appartient à qui photographie l'écran — d'où le code.",
    "Choose ntfy instead and the body is a line of prose with a title and a priority — which is all a push server needs to put the message on a phone's lock screen. The URL is then your topic, say https://ntfy.sh/my-secret-topic or your own server's; anyone who knows the topic can read it, so make it long and private.": "Choisissez ntfy et le corps devient une phrase avec un titre et une priorité — tout ce qu'un serveur de push demande pour afficher le message sur l'écran verrouillé. L'URL est alors votre sujet, par exemple https://ntfy.sh/mon-sujet-secret ou celui de votre propre serveur ; qui connaît le sujet peut lire, alors faites-le long et privé.",
    'JSON, signed': 'JSON, signé',
    'New code': 'Nouveau code',
    'Pair a device': 'Appairer un appareil',
    'Pair a phone or a tablet': 'Appairer un téléphone ou une tablette',
    'Pairing code {code} — good for five minutes, and for one device.': "Code d'appairage {code} — valable cinq minutes et pour un appareil.",
    'Speaks': 'Parle',
    'The address to type into the app:': "L'adresse à saisir dans l'app :",
    ...
    'A month': 'Par mois',
    'An average over months you have only half recorded is half an average; pick a window you trust.': 'Une moyenne sur des mois à moitié enregistrés est une demi-moyenne ; choisissez une période en laquelle vous avez confiance.',
    'Over': 'Sur',
    'That is not a transaction.': "Ce n'est pas une transaction.",
    'The big ones': 'Les gros postes',
    'a single payment can be left out of the monthly figures, or spread over the years it is used — the amount on the row does not change': "un paiement unique peut être laissé hors des chiffres mensuels ou réparti sur les années d'usage — le montant de la ligne ne change pas",
    'its own month': 'dans son mois',
    'left out': 'laissé de côté',
    'over {n} month': 'sur {n} mois',
    'over {n} months': 'sur {n} mois',
    '{n} year': '{n} an',
    '{n} years': '{n} ans',
    'Nothing matches those terms today. Check the spelling against the description as the Transactions page shows it, and remember that the direction, the amount range, the account and the kind all have to fit too.': "Rien ne correspond à ces critères aujourd'hui. Vérifiez l'orthographe par rapport au libellé tel que la page Transactions l'affiche, et n'oubliez pas que le sens, la plage de montant, le compte et le type doivent également convenir.",
    '{n} rows match, and every one of them is a purchase or a sale. A rule does not file trades unless it says which account or which kind it means — pick the account (or the kind) in the rule, and it will.': '{n} lignes correspondent, et toutes sont un achat ou une vente. Une règle ne classe les opérations que si elle dit quel compte ou quel type elle vise — choisissez le compte (ou le type) dans la règle, et elle le fera.',
    '{n} of the matching rows are purchases or sales and were left alone: a rule files trades only when it names an account or a kind.': "{n} des lignes correspondantes sont des achats ou des ventes et ont été laissées telles quelles : une règle ne classe les opérations que lorsqu'elle nomme un compte ou un type.",
