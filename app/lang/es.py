...
STRINGS: dict[str, str] = {
    "Documents from Paperless": "Documentos de Paperless",
    "This account's documents": "Los documentos de esta cuenta",
    "See what matches": "Ver qué coincide",
    "What this would pull": "Lo que se traería",
    "{n} document in the archive matches": "{n} documento del archivo coincide",
    "{n} documents in the archive match": "{n} documentos del archivo coinciden",
    "Document": "Documento",
    "Filed": "Archivado",
    "State": "Estado",
    "already read": "ya leído",
    "new": "nuevo",
    "The first {shown} of {matched} are shown.": "Se muestran los primeros {shown} de {matched}.",
    "Nothing in the archive matches all of these at once.": "Nada en el archivo cumple todo esto a la vez.",
    "Leave them all blank and this account pulls nothing.": "Si lo deja todo vacío, esta cuenta no trae nada.",
    "Which documents in the archive belong to {account}. Everything named has to hold at once — every tag, that correspondent, that type — so the pull can be surgical. The daily sync fetches whatever is new; nothing is read twice.": "Qué documentos del archivo son de {account}. Todo lo indicado debe cumplirse a la vez — cada etiqueta, ese corresponsal, ese tipo — para traer justo lo que quiere. La sincronización diaria trae lo nuevo; nada se lee dos veces.",
    "Which Paperless documents are this account's — tags, correspondent, type — is set where it is used: {link}.": "Qué documentos de Paperless son de esta cuenta — etiquetas, corresponsal, tipo — se define donde se usa: {link}.",
    "This account does not say which documents are its yet — name the tags, correspondent or type here.": "Esta cuenta aún no dice qué documentos son suyos: indique aquí etiquetas, corresponsal o tipo.",
    ...
    "Rules": "Reglas",
    "Re-apply every rule": "Volver a aplicar todas las reglas",
    "Every rule re-applied: {n} transaction filed.": "Todas las reglas aplicadas de nuevo: {n} transacción clasificada.",
    "Every rule re-applied: {n} transactions filed.": "Todas las reglas aplicadas de nuevo: {n} transacciones clasificadas.",
    "not a subscription": "no es una suscripción",
    "it is a subscription": "sí es una suscripción",
    "put it back": "devolverla",
    "Dismissed": "Descartadas",
    "you said these are not subscriptions; they stay out of the totals": "usted dijo que no son suscripciones; quedan fuera de los totales",
    ...
    'Statement formats': 'Formatos de extracto',
    "CAMT.053 / CAMT.052 XML (the ISO 20022 export of every European online-banking portal), MT940 (.sta, SWIFT) and OFX / QFX (North American and British banks and brokers) are read as they are — no mapping, the bank's own reference as the id, the closing balance taken.": 'CAMT.053 / CAMT.052 XML (la exportación ISO 20022 de toda banca en línea europea), MT940 (.sta, SWIFT) y OFX / QFX (bancos y brókeres norteamericanos y británicos) se leen tal cual — sin asignación, la referencia del propio banco como id, el saldo final tomado.',
    ...
    "Today's cash, carried forward through what is known to be coming: the bills as declared, the subscriptions the app has found, the salary as the newest payslip had it. Where the balance gets lowest, and the day it would cross zero — before it does, not after.": 'El efectivo de hoy, proyectado a través de lo que se sabe que viene: las facturas tal como se declararon, las suscripciones que la app ha encontrado, el sueldo tal como lo daba la última nómina. Dónde el saldo llega más bajo, y el día en que cruzaría el cero: antes de que ocurra, no después.',
    'Cash today': 'Efectivo hoy',
    '{n} cash account': '{n} cuenta corriente',
    '{n} cash accounts': '{n} cuentas corrientes',
    'newest reading each': 'la lectura más reciente de cada una',
    'In {n} days': 'En {n} días',
    '{inflow} in, {outflow} out': '{inflow} entran, {outflow} salen',
    'Lowest point': 'Punto más bajo',
    'below zero on {date}, after {name}': 'bajo cero el {date}, tras {name}',
    'on {date}, after {name}': 'el {date}, tras {name}',
    'starts below zero — an overdraft or a card, not a crossing': 'empieza bajo cero: un descubierto o una tarjeta, no un cruce',
    'nothing scheduled': 'nada previsto',
    'Scheduled': 'Previsto',
    '{bills} bills, {subs} subscriptions, {income} pay days': '{bills} facturas, {subs} suscripciones, {income} días de pago',
    'No exchange rate for {currencies}; what is in them is left out of the figures above.': 'Sin tipo de cambio para {currencies}; lo que hay en esas monedas queda fuera de las cifras de arriba.',
    'Balance ahead': 'Saldo por delante',
    '{n} days': '{n} días',
    'Nothing is scheduled in this window. Declare a bill, or import a payslip, and the line starts moving.': 'No hay nada previsto en este plazo. Declara una factura, o importa una nómina, y la línea empieza a moverse.',
    'Bills →': 'Facturas →',
    'Day by day': 'Día a día',
    'income lands before bills on the same day · ~ an amount carried from what it last cost': 'los ingresos llegan antes que las facturas el mismo día · ~ un importe tomado de lo que costó la última vez',
    'bill': 'factura',
    'subscription': 'suscripción',
    'missed — expected today': 'no pagada: se espera hoy',
    'Starting from': 'Punto de partida',
    'No cash account yet.': 'Aún no hay ninguna cuenta corriente.',
    'Loan instalments count only when declared as a bill: the app knows what a loan costs, not which account pays it.': 'Las cuotas de un préstamo solo cuentan si están declaradas como factura: la app sabe lo que cuesta un préstamo, no qué cuenta lo paga.',
    ...
    "A POST to a URL of yours when something happened: a sync ran or failed, a bill is past due. Home Assistant, n8n, a bot, a script. The body is JSON — event, time, data — and the X-Wealth-Signature header is an HMAC-SHA256 of it with the receiver's secret, so it can tell this app from anyone who found the URL. One attempt, five seconds; a receiver that is down misses that event and the list says so.": 'Un POST a una URL tuya cuando ha pasado algo: una sincronización se ejecutó o falló, una factura está vencida. Home Assistant, n8n, un bot, un script. El cuerpo es JSON — evento, hora, datos — y la cabecera X-Wealth-Signature es un HMAC-SHA256 de él con el secreto del receptor, para que distinga esta app de cualquiera que haya encontrado la URL. Un intento, cinco segundos; un receptor caído se pierde ese evento y la lista lo dice.',
    'Add the webhook': 'Añadir el webhook',
    'Create a token above and the examples appear here.': 'Crea un token arriba y aquí aparecerán los ejemplos.',
    'Events': 'Eventos',
    'Every tool the assistant has is also a URL, for a script or an automation that speaks no MCP — the same token, the same answers, one registry. GET lists the tools with their schemas; GET or POST calls one, arguments as query parameters or a JSON body.': 'Cada herramienta del asistente es también una URL, para un script o una automatización que no hable MCP — el mismo token, las mismas respuestas, un solo registro. GET lista las herramientas con sus esquemas; GET o POST llama a una, argumentos como parámetros de consulta o cuerpo JSON.',
    'Last': 'Último',
    'REST API': 'API REST',
    'Secret': 'Secreto',
    'Send a test event': 'Enviar un evento de prueba',
    'Test event sent to {n} webhook.': 'Evento de prueba enviado a {n} webhook.',
    'Test event sent to {n} webhooks.': 'Evento de prueba enviado a {n} webhooks.',
    'Webhook added. Its secret is shown in the list; give it to the receiver to check the signature.': 'Webhook añadido. Su secreto aparece en la lista; dáselo al receptor para comprobar la firma.',
    'Webhook removed.': 'Webhook eliminado.',
    'Webhooks': 'Webhooks',
    'the same tools over plain HTTP': 'las mismas herramientas por HTTP simple',
    '{n} receiver': '{n} receptor',
    '{n} receivers': '{n} receptores',
    # Undoing an import (0.36.1)
    "A named kind decides the sign: a Kauf is money out whichever way the bank wrote the figure, a Dividende money in. Without a kind column the sign is the file's — so if your bank writes a purchase as a positive amount and has no kind column, tick the box below, or the buys come in as money received.": 'Un tipo con nombre decide el signo: un Kauf es dinero saliente como quiera que el banco escribiera la cifra, un Dividende dinero entrante. Sin columna de tipo, el signo es el del archivo — así que si tu banco escribe una compra en positivo y no tiene columna de tipo, marca la casilla de abajo, o las compras entran como dinero recibido.',
    'An import can be taken back as one thing — every row it brought, and only those; a row a re-import found already there belongs to the import that first brought it. Wrong signs from a mapping: undo, forget the mapping, import again.': 'Una importación se puede deshacer como un todo — cada fila que trajo, y solo esas; una fila que una reimportación encontró ya presente pertenece a la importación que la trajo primero. Signos erróneos de una correspondencia: deshacer, olvidar la correspondencia, importar de nuevo.',
    'File': 'Archivo',
    'Import undone — {n} row removed.': 'Importación deshecha — {n} fila eliminada.',
    'Import undone — {n} rows removed.': 'Importación deshecha — {n} filas eliminadas.',
    'Recent imports': 'Importaciones recientes',
    'Remove every row this import brought?': '¿Eliminar cada fila que trajo esta importación?',
    'Rows brought': 'Filas traídas',
    'Signs.': 'Signos.',
    'That import is not on record.': 'Esa importación no consta.',
    'That is usually the signs the wrong way round.': 'Eso suelen ser los signos al revés.',
    'Through': 'Mediante',
    'Undo': 'Deshacer',
    'and forget its mapping': 'y olvidar su correspondencia',
    'nothing left of it': 'no queda nada de ella',
    'of {n}': 'de {n}',
    '{n} of the first rows look like purchases with money coming in.': '{n} de las primeras filas parecen compras con dinero entrante.',
    '{n} of the first rows looks like a purchase with money coming in.': '{n} de las primeras filas parece una compra con dinero entrante.',
    # Retirement plan and goal kinds (0.37.0)
    'A car': 'Un coche',
    'A deposit for a place of your own.': 'Una entrada para una casa propia.',
    'A home': 'Una casa',
    'A month until then': 'Al mes hasta entonces',
    'A trip': 'Un viaje',
    'A wedding': 'Una boda',
    'An amount by a date, for whatever it is.': 'Un importe para una fecha, sea para lo que sea.',
    'An emergency fund': 'Un fondo de emergencia',
    "An item with no ages runs from retirement to the horizon; clear its name to remove it. Amounts are in today's money and inflate with the rate above; what the pile has to cover is spending minus income, grossed up for the tax on withdrawals.": 'Un concepto sin edades va desde la jubilación hasta el horizonte; borra su nombre para quitarlo. Los importes están en dinero de hoy y suben con la tasa de inflación de arriba; lo que el capital debe cubrir es el gasto menos los ingresos, aumentado por el impuesto sobre las retiradas.',
    'Fees, % a year': 'Comisiones, % al año',
    'Fees, books and courses.': 'Tasas, libros y cursos.',
    'For': 'Para',
    'Funded': 'Cubierto',
    "Income in retirement, a month, in today's money": 'Ingresos en la jubilación, al mes, en dinero de hoy',
    'Inflation, %': 'Inflación, %',
    'Lasts to {age}': 'Dura hasta los {age}',
    'Life after work — and whether it lasts. A plan of its own.': 'La vida después del trabajo — y si alcanza. Un plan aparte.',
    'Living, health, travel…': 'Vida, salud, viajes…',
    'Nominal': 'Nominal',
    'On these assumptions the pile is {value} when {name} retires at {retire} in {year}, against {required} needed — the money runs out at {age}, {years} years before the horizon.': 'Con estos supuestos el capital es {value} cuando {name} se jubila a los {retire} en {year}, frente a {required} necesarios — el dinero se acaba a los {age}, {years} años antes del horizonte.',
    'On these assumptions the pile is {value} when {name} retires at {retire} in {year}, against {required} needed, and {left} is still there at {horizon}.': 'Con estos supuestos el capital es {value} cuando {name} se jubila a los {retire} en {year}, frente a {required} necesarios, y a los {horizon} aún quedan {left}.',
    'Plan saved.': 'Plan guardado.',
    'Plan to age': 'Planificar hasta los',
    'Projected': 'Proyectado',
    'Required': 'Necesario',
    'Required is, from retirement on, the capital that funds the rest of the plan from that year at the return after retirement; before it, the path that would reach the required capital exactly with the same contributions — the projection above it means ahead of plan. Projections rest on your assumptions; outcomes will differ. Not financial advice.': 'Necesario es, desde la jubilación, el capital que financia el resto del plan desde ese año al rendimiento posterior; antes, la senda que alcanzaría exactamente el capital necesario con las mismas aportaciones — la proyección por encima significa ir por delante del plan. Las proyecciones se basan en tus supuestos; la realidad diferirá. No es asesoramiento financiero.',
    'Retired already': 'Ya jubilado',
    'Retirement': 'Jubilación',
    'Retirement plan': 'Plan de jubilación',
    'Return after, %': 'Rendimiento después, %',
    'Return before, %': 'Rendimiento antes, %',
    'Runs out at {age}': 'Se acaba a los {age}',
    'Save the plan': 'Guardar el plan',
    'Something I am saving for': 'Algo para lo que ahorro',
    "Spending in retirement, a month, in today's money": 'Gastos en la jubilación, al mes, en dinero de hoy',
    'State pension, rent, a part-time job…': 'Pensión pública, alquiler, un trabajo a tiempo parcial…',
    'Tax on withdrawals, %': 'Impuesto sobre retiradas, %',
    'The day itself.': 'El día en sí.',
    'The next car, paid for rather than financed.': 'El próximo coche, pagado en vez de financiado.',
    'The one you keep putting off.': 'El que siempre pospones.',
    'The plan': 'El plan',
    'Three to six months of spending, untouched.': 'De tres a seis meses de gastos, intactos.',
    "Today's money": 'Dinero de hoy',
    "Will the money last — and if not, until when. What goes in until you stop, what you will spend each month from then on, what will still come in, a return before and after, fees, inflation, tax on what is withdrawn: walked a year at a time to a horizon. The Forecast page's outlook is the first answer; this is the one with the retirement in it.": '¿Alcanzará el dinero — y si no, hasta cuándo? Lo que entra hasta que pares, lo que gastarás cada mes desde entonces, lo que seguirá entrando, un rendimiento antes y después, comisiones, inflación, impuesto sobre las retiradas: recorrido año a año hasta un horizonte. La perspectiva de la página Previsión es la primera respuesta; esta es la que lleva la jubilación dentro.',
    'Withdrawn': 'Retirado',
    'age {age}': '{age} años',
    'every rate is yours; the page predicts nothing': 'cada tasa es tuya; la página no predice nada',
    'from age': 'desde los',
    'from the Forecast plan': 'del plan de Previsión',
    'from {value} today across {n} accounts, {monthly} a month{plan}': 'desde {value} hoy en {n} cuentas, {monthly} al mes{plan}',
    "in today's money": 'en dinero de hoy',
    'nominal': 'nominal',
    'retired': 'jubilado',
    'saving': 'ahorro',
    'to age': 'hasta los',
    '{monthly} a month needed at retirement, in the money of that day': '{monthly} al mes necesarios al jubilarse, en dinero de ese día',
    '…growing % a year': '…creciendo % al año',
    # Moving in from Financial Planner (0.42.0)
    'Move in': 'Mudarse',
    'Move in from Financial Planner': 'Mudarse desde Financial Planner',
    'Moved in': 'Mudanza hecha',
    'Bring the books of the Financial Planner app over in one go: its accounts, every row of its ledger, its holdings, the daily balances it snapshotted and the price history it kept. Upload its wealth.db, look at what will be written, then confirm — nothing is touched until you do.': 'Trae los libros de la app Financial Planner de una vez: sus cuentas, cada fila de su libro, sus posiciones, los saldos diarios de sus instantáneas y su historial de precios. Sube su wealth.db, mira lo que se escribirá y confirma — hasta entonces no se toca nada.',
    'Accounts created': 'Cuentas creadas',
    'Rows': 'Filas',
    'Rows already here': 'Filas ya presentes',
    'Opening positions': 'Posiciones de apertura',
    'Balance readings': 'Lecturas de saldo',
    'Price days': 'Días de precio',
    'Categories created': 'Categorías creadas',
    "Each account's rows are one import, so an account page can undo its share. Refresh the prices and the rates next — Settings › Market — and compare the overview with the old app's.": 'Las filas de cada cuenta son una importación, así que la página de una cuenta puede deshacer su parte. Actualiza después precios y tipos — Ajustes › Mercado — y compara el resumen con la app antigua.',
    'To the overview': 'Al resumen',
    'What will be written': 'Lo que se escribirá',
    '{rows} rows across {accounts} accounts, {balances} balance readings, {openings} opening positions, {prices} price days, {cats} new categories.': '{rows} filas en {accounts} cuentas, {balances} lecturas de saldo, {openings} posiciones de apertura, {prices} días de precio, {cats} categorías nuevas.',
    '{n} rows are already here and are left alone.': '{n} filas ya están aquí y se dejan como están.',
    'Old account': 'Cuenta antigua',
    'From — to': 'De — a',
    'Readings': 'Lecturas',
    'Goes into': 'Va a',
    'closed': 'cerrada',
    'left out — holds nothing': 'se deja fuera — no contiene nada',
    'a new account': 'una cuenta nueva',
    'Where the rows do not add up to what the old app held — a position set from a statement, a sale whose purchase predates the ledger — a row dated before the first one makes up the difference, at the cost the old app carried.': 'Donde las filas no suman lo que la app antigua tenía — una posición fijada desde un extracto, una venta cuya compra es anterior al libro — una fila fechada antes de la primera cubre la diferencia, al coste que llevaba la app antigua.',
    'held {held}, rows give {rows}': 'en cartera {held}, las filas dan {rows}',
    'New categories:': 'Categorías nuevas:',
    'Worth knowing': 'Conviene saber',
    "Rows keep the ids the old app gave them. Where this app's own importer would give the same row a different id, the account remembers how far its ledger is on record, and an export or a sync covering those days leaves them alone — so an export from before today need not be imported again, and a later one is safe.": 'Las filas conservan los identificadores de la app antigua. Donde el importador de esta app daría otro identificador a la misma fila, la cuenta recuerda hasta dónde está registrado su libro, y una importación o sincronización que cubra esos días las deja en paz — así que una exportación anterior a hoy no hay que volver a importarla, y una posterior es segura.',
    'The wealth.db file': 'El archivo wealth.db',
    'Read it': 'Leerlo',
    "Take a copy of the old app's wealth.db while it is not running, and upload the copy. Nothing is written until you have seen the plan and confirmed it.": 'Copia la wealth.db de la app antigua mientras no esté en marcha y sube la copia. No se escribe nada hasta que hayas visto el plan y confirmado.',
    'That upload has expired — start again.': 'Esa subida ha caducado — empieza de nuevo.',
    'Moved in: {rows} rows, {balances} balance readings, {accounts} accounts created.': 'Mudanza hecha: {rows} filas, {balances} lecturas de saldo, {accounts} cuentas creadas.',
    'That is not a Financial Planner database — it should be the wealth.db file.': 'Eso no es una base de datos de Financial Planner — debería ser el archivo wealth.db.',
    'Could not read the file: {reason}': 'No se pudo leer el archivo: {reason}',
    'Ledger on record until': 'Libro registrado hasta',
    'Rows up to this day were moved in from another app under its own ids. An import or a sync covering those days leaves them alone, so nothing is booked twice. Clear it to let everything in.': 'Las filas hasta este día se trajeron de otra app con sus propios identificadores. Una importación o sincronización que cubra esos días las deja en paz, para que nada se contabilice dos veces. Vacíalo para dejar entrar todo.',
    'Worked out for the accounts proposed above, and again for the ones you pick when you confirm: rows already in a chosen account count towards its holding.': 'Calculado para las cuentas propuestas arriba, y de nuevo para las que elijas al confirmar: las filas que ya estén en una cuenta elegida cuentan para su posición.',
    # Trades on the chart, sold-out list, removing a row (0.43.0)
    'A correction survives the next import: the row is recognised by its id and left as you set it. So does a removal — the id is remembered, and an import or a sync leaves that row out.': 'Una corrección sobrevive a la siguiente importación: la fila se reconoce por su identificador y se deja como la pusiste. Una eliminación también — el identificador se recuerda, y una importación o sincronización deja esa fila fuera.',
    "A position sold down to nothing leaves the table above; it is kept here so what it made does not vanish with it. Its rows are on its page, like any other's.": 'Una posición vendida hasta cero sale de la tabla de arriba; se conserva aquí para que lo que dio no desaparezca con ella. Sus filas están en su página, como las de cualquier otra.',
    'Bought': 'Comprado',
    'Dividend': 'Dividendo',
    'Mark the trades': 'Marcar las operaciones',
    'Remove this row? An import or a sync will not bring it back.': '¿Eliminar esta fila? Una importación o sincronización no la traerá de vuelta.',
    'Removed. An import or a sync will not bring it back.': 'Eliminada. Una importación o sincronización no la traerá de vuelta.',
    'Sold': 'Vendido',
    'Sold out': 'Vendidos por completo',
    'Split': 'Split',
    'That row is not there.': 'Esa fila no existe.',
    ...
    'incl.': 'incl.',
    'Cash & banks': 'Efectivo y bancos',
    'Investments': 'Inversiones',
    'Pension': 'Pensión',
    'Liabilities': 'Pasivos',
    'In its currency': 'En su divisa',
    'what each is worth, cash and holdings together': 'lo que vale cada una, efectivo y posiciones juntos',
    '{n} row here': '{n} fila aquí',
    ...
    # ─── The loan's own page ─────────────────────────────────────────
    'A loan on its schedule: {balance} still owed, paid off {date}.': 'Un préstamo según su cuadro: quedan {balance}, liquidado el {date}.',
    "A {currency} loan on its fixed {rhythm} schedule from the first instalment of {first} until it is paid off. Every figure is computed from the terms — a constant annuity, interest rounded to the cent each period, the way a bank does it — and reproduces the bank's own breakdown.": 'Un préstamo en {currency} según su cuadro fijo ({rhythm}), desde la primera cuota del {first} hasta su liquidación. Cada cifra se calcula a partir de las condiciones — una anualidad constante, intereses redondeados al céntimo en cada periodo, como hace un banco — y reproduce el desglose del propio banco.',
    'Amortisation schedule': 'Cuadro de amortización',
    'At this rate the loan is overwhelmingly principal from day one — the interest bars are barely visible, which is exactly why the balance falls almost linearly.': 'A este tipo, el préstamo es casi todo capital desde el primer día — las barras de intereses apenas se ven, y por eso el saldo baja de forma casi lineal.',
    'Drawn as': 'Dispuesto como',
    'Every payment is a fixed annuity: interest, shrinking, against capital repaid.': 'Cada pago es una anualidad fija: intereses, decrecientes, frente al capital amortizado.',
    'From {amount} at drawdown to zero on {date}.': 'De {amount} en la disposición a cero el {date}.',
    'Original loan ({year})': 'Préstamo original ({year})',
    'Outstanding': 'Pendiente',
    'Outstanding balance over time': 'Saldo pendiente a lo largo del tiempo',
    "From the reading of {date} the schedule runs from {amount}, the lender's figure, not the sum's — a rate change, a fee, a rounding rule all live in that number. Record a newer balance on the account and it takes over from there.": 'Desde la lectura del {date} el cuadro parte de {amount}, la cifra del prestamista y no la de la suma: un cambio de tipo, una comisión, una regla de redondeo viven todos en ese número. Registra un saldo más reciente en la cuenta y tomará el relevo desde ahí.',
    'from the reading of {date}': 'desde la lectura del {date}',
    'From here the schedule runs from the reading of {date}.': 'Desde aquí el cuadro parte de la lectura del {date}.',
    'reading': 'lectura',
    'Outstanding today': 'Pendiente hoy',
    'Payment': 'Pago',
    'Payoff date': 'Fecha de liquidación',
    'Repaid so far': 'Amortizado hasta hoy',
    'Show amounts in:': 'Mostrar importes en:',
    'That account already has its terms.': 'Esa cuenta ya tiene sus condiciones.',
    'That account is not a loan.': 'Esa cuenta no es un préstamo.',
    'The dashed markers show today and the final payoff. The balance steps down on each payment date; because the interest share shrinks as the balance falls, the curve gently accelerates toward zero.': 'Las marcas discontinuas señalan hoy y la última cuota. El saldo baja un escalón en cada fecha de pago; como la parte de intereses se reduce con el saldo, la curva se acelera suavemente hacia cero.',
    "The loan is in {currency}; {base} is shown for reference only, at today's rate.": 'El préstamo está en {currency}; {base} se muestra solo como referencia, al tipo de hoy.',
    'The loan over time →': 'El préstamo a lo largo del tiempo →',
    'The terms are on record: the balance follows the schedule from here, and its history runs from the first instalment.': 'Las condiciones están registradas: el saldo sigue el cuadro desde ahora, y su historial arranca en la primera cuota.',
    'This is a loan. Give it its terms': 'Esto es un préstamo. Dale sus condiciones',
    'Today': 'Hoy',
    'Total interest over the whole life of the loan: {amount}.': 'Intereses totales durante toda la vida del préstamo: {amount}.',
    'Where each instalment goes': 'A dónde va cada cuota',
    'and its balance follows the schedule, with the page a mortgage deserves': 'y su saldo seguirá el cuadro, con la página que una hipoteca merece',
    "at today's rate of {rate} {base} per {currency}": 'al tipo de hoy de {rate} {base} por {currency}',
    'change the terms': 'cambiar las condiciones',
    'faded rows are still in the future': 'las filas atenuadas aún están en el futuro',
    'the loan is denominated in {currency}': 'el préstamo está denominado en {currency}',
    '{amount} @ {rate} on drawdown': '{amount} @ {rate} en la disposición',
    '{currency} (loan currency)': '{currency} (moneda del préstamo)',
    "{currency} (today's rate)": '{currency} (tipo de hoy)',
    '{name} — loan over time': '{name} — el préstamo a lo largo del tiempo',
    '{n} instalment': '{n} cuota',
    '{n} instalments': '{n} cuotas',
    '{n} left': '{n} restantes',
    '{pct} % of the original': '{pct} % del original',
    '{rate} % per year': '{rate} % anual',
    'The instalment is the figure on the contract; the rate the nominal one per year. “Drawn as” is what the loan was worth in another currency the day it began — a CHF mortgage taken out as €150 000 — and stays on the page as the reference it is.': 'La cuota es la cifra del contrato; el tipo, el nominal anual. «Dispuesto como» es lo que valía el préstamo en otra moneda el día que empezó — una hipoteca en CHF contratada como 150 000 € — y queda en la página como la referencia que es.',

    # ─── Income: payslips ────────────────────────────────────────────
    "A payslip is imported into the account the net salary lands in: open that account, Import a file, choose the PDF — several at once is fine. The sheet is kept whole here, and what the bank never sees is booked onto the account, zero-sum: the tax at source and each side's pension contribution, grossed up and then paid or invested. The net itself is left to the bank.": 'Una nómina se importa en la cuenta donde llega el salario neto: abre esa cuenta, Importar un archivo, elige el PDF — varios a la vez está bien. La hoja se guarda entera aquí, y lo que el banco nunca ve se registra en la cuenta, con suma cero: el impuesto en origen y la aportación a la pensión de cada parte, en bruto y luego pagados o invertidos. El neto se deja al banco.',
    'Adding a payslip': 'Añadir una nómina',
    'Bonus': 'Bonus',
    'Employer pension': 'Pensión empresa',
    "Employer's cost": 'Coste para la empresa',
    'Gross': 'Bruto',
    'Month': 'Mes',
    'Net paid': 'Neto pagado',
    'Payslip removed, with the rows it had booked.': 'Nómina eliminada, con las filas que había registrado.',
    'Pension, both sides': 'Pensión, ambas partes',
    'Remove this payslip and the rows it booked?': '¿Eliminar esta nómina y las filas que registró?',
    'Social': 'Cargas sociales',
    'Tax at source': 'Impuesto en origen',
    'That payslip is not there.': 'Esa nómina no existe.',
    "The household's payslips: gross pay, what was taken at source, the pension on both sides, the bonuses — per earner. The bank saw the net arrive; the sheet saw the rest, and that is what a tax return and a retirement forecast ask for.": 'Las nóminas del hogar: el bruto, lo retenido en origen, la pensión por ambas partes, los bonus — por perceptor. El banco vio llegar el neto; la hoja vio el resto, y eso es lo que piden una declaración de la renta y una previsión de jubilación.',
    "Two layouts are read: the SAP Lohnabrechnung of a large employer, with wage codes, and the small employer's sheet with Bruttolohn, AHV, ALV, BVG and Nettolohn. The earner's name is taken from the sheet, so one household's payslips sort themselves.": 'Se leen dos formatos: la Lohnabrechnung SAP de una gran empresa, con códigos salariales, y la hoja del pequeño empleador con Bruttolohn, AHV, ALV, BVG y Nettolohn. El nombre del perceptor se toma de la hoja, así que las nóminas de un hogar se ordenan solas.',
    'Your accounts a salary could land in:': 'Tus cuentas donde podría llegar un salario:',
    'bonus {amount}': 'bonus {amount}',
    "gross plus the employer's contributions": 'bruto más las aportaciones de la empresa',
    'gross {amount}': 'bruto {amount}',
    'of {gross} gross': 'de {gross} brutos',
    'paid {date}': 'pagado el {date}',
    'pension inflow {amount}': 'entrada a la pensión {amount}',
    'tax {pct} %': 'impuesto {pct} %',
    'the statutory floor — the sheet prints no employer block': 'el mínimo legal — la hoja no imprime bloque de empresa',
    "{mine} yours, {theirs} the employer's": '{mine} tuyos, {theirs} de la empresa',
    '{n} month': '{n} mes',
    '{n} months': '{n} meses',
    '{pct} % of gross': '{pct} % del bruto',

    # ─── The payslip mapper ──────────────────────────────────────────
    'A sheet naming one of these employers and earners is read through its mapping without asking. Forget one and the next such sheet asks again — the payslips already imported stay.': 'Una hoja que nombre a uno de estos empleadores y perceptores se lee con su correspondencia sin preguntar. Olvida una y la siguiente hoja así vuelve a preguntar — las nóminas ya importadas se quedan.',
    'Allowance': 'Complemento',
    'Amounts': 'Importes',
    'Base salary': 'Salario base',
    "Both names must appear on the sheet as written here — spaces and case do not matter — because that is how the next sheet is recognised. The earner's name is what the Income page groups by.": 'Ambos nombres deben aparecer en la hoja tal como se escriben aquí — espacios y mayúsculas no importan — porque así se reconoce la siguiente hoja. El nombre del perceptor es por lo que agrupa la página de Ingresos.',
    'Earner': 'Perceptor',
    'Employer': 'Empleador',
    "Employer's side": 'Parte de la empresa',
    'Label': 'Concepto',
    'Lines': 'Líneas',
    'Map the payslip': 'Asignar la nómina',
    'Mapping forgotten. The next sheet from that employer asks again.': 'Correspondencia olvidada. La siguiente hoja de ese empleador vuelve a preguntar.',
    'Means': 'Significa',
    "No importer here knows this sheet, and it does not need to: say which line is which, once. The mapping is kept under the sheet's own markers — the employer and the earner — so next month's sheet from the same employer is recognised by itself. The suggestions come from a catalogue of what payslips call things in the languages they are printed in; what is remembered is what you confirm.": 'Ningún importador conoce esta hoja, y no hace falta: di una vez qué línea es qué. La correspondencia se guarda bajo las marcas propias de la hoja — el empleador y el perceptor — de modo que la hoja del mes siguiente del mismo empleador se reconoce sola. Las sugerencias vienen de un catálogo de cómo llaman las nóminas a las cosas, en los idiomas en que se imprimen; lo que se recuerda es lo que tú confirmas.',
    'On a line with several amounts the last one is the amount; the ones before it are the basis and the rate. Deductions are stored as money out whichever way the sheet prints them — a French bulletin prints its retenues positive, an SAP sheet with a trailing minus — because what a line means settles the sign. Several lines may mean the same thing: AHV and ALV are both social deductions. Lines left at — are kept as detail and count for nothing.': 'En una línea con varios importes el último es el importe; los anteriores son la base y el tipo. Las deducciones se guardan como salida de dinero se impriman como se impriman — un boletín francés imprime sus retenciones en positivo, una hoja SAP con un menos al final — porque lo que significa una línea decide el signo. Varias líneas pueden significar lo mismo: AHV y ALV son ambas cargas sociales. Las líneas dejadas en — se conservan como detalle y no cuentan.',
    'Other deduction': 'Otra deducción',
    'Paid on': 'Pagado el',
    'Payslip mappings': 'Correspondencias de nóminas',
    "Pension, the employer's": 'Pensión, de la empresa',
    'Pension, yours': 'Pensión, tuya',
    'Rate': 'Tipo',
    'Read it through this mapping': 'Leerla con esta correspondencia',
    'Read through the mapping': 'Leída con la correspondencia',
    'Social deduction': 'Carga social',
    "Social, the employer's": 'Carga social, de la empresa',
    'Taken at source': 'Retenido en origen',
    'The gross and the net paid, the employer and the earner — that is the least a payslip mapping needs.': 'El bruto y el neto pagado, el empleador y el perceptor — es lo mínimo que necesita una correspondencia de nómina.',
    'The lines': 'Las líneas',
    'The mapping is saved as {name}; the next sheet from this employer is recognised by itself.': 'La correspondencia se guarda como {name}; la siguiente hoja de este empleador se reconoce sola.',
    'The month': 'El mes',
    'This is not a payslip': 'Esto no es una nómina',
    'Whose sheet': 'De quién es la hoja',
    'amounts written the {fmt} way': 'importes escritos como {fmt}',
    'base {amount}': 'base {amount}',
    'gross less every deduction is the net: the mapping caught every line': 'el bruto menos cada deducción da el neto: la correspondencia captó todas las líneas',
    'gross less the deductions differs from the net by {gap} — a line is missing or mapped twice': 'el bruto menos las deducciones difiere del neto en {gap} — falta una línea o hay una contada dos veces',
    "nothing mapped as the employer's — the sheet may print none": 'nada asignado a la parte de la empresa — puede que la hoja no la imprima',
    'pension {pension} · social {social}': 'pensión {pension} · social {social}',
    'tax {tax} · social {social} · pension {pension} · other {other}': 'impuesto {tax} · social {social} · pensión {pension} · otro {other}',
    '{n} line with an amount, a month or a date': '{n} línea con un importe, un mes o una fecha',
    '{n} lines with an amount, a month or a date': '{n} líneas con un importe, un mes o una fecha',
    '{n} payslip layout you mapped yourself': '{n} formato de nómina que asignaste tú',
    '{n} payslip layouts you mapped yourself': '{n} formatos de nómina que asignaste tú',
    "Row id":
        "Id de fila",
    ...
    "587 for STARTTLS, 465 for TLS":
        "587 para STARTTLS, 465 para TLS",
    "7d is the price move of what you hold now, in the account's currency — a deposit or a purchase during the week is not a gain. Gain since purchase compares today's value with what you put in. The net worth line at the top is the balance, and moves when money arrives too.":
        "7d es el movimiento de precio de lo que tienes ahora, en la moneda de la cuenta — un ingreso o una compra durante la semana no es una ganancia. La ganancia desde la compra compara el valor de hoy con lo que aportaste. La línea del patrimonio de arriba es el saldo, y también se mueve cuando llega dinero.",
    "Dividends, 12 months":
        "Dividendos, 12 meses",
    "Dividends, last {n} days":
        "Dividendos de los últimos {n} días",
    "Forget the e-mail settings":
        "Olvidar los ajustes de correo",
    "Friday":
        "Viernes",
    "Gain since purchase":
        "Ganancia desde la compra",
    "Holding":
        "Posición",
    "Indices":
        "Índices",
    ...
    "No dividends in the last {n} days.":
        "Sin dividendos en los últimos {n} días.",
    "Not in the dividend total, no rate on file: {amounts}":
        "Fuera del total de dividendos, sin tipo de cambio registrado: {amounts}",
    "On":
        "El",
    "Once a week, a mail with the week: net worth and its change, every broker and crypto account with its value, price move, gain since purchase, holdings, dividends of the last thirty days and the trailing year, and the indices the app tracks. Sent through an SMTP server of yours — Gmail with an app password works — after the daily sync on the day you pick. It is the one thing this app sends anywhere but your bank: to your own mailbox. The password is kept beside the bank key.":
        "Una vez por semana, un correo con la semana: el patrimonio y su variación, cada cuenta de bróker y cripto con su valor, movimiento de precio, ganancia desde la compra, posiciones, dividendos de los últimos treinta días y del último año, y los índices que la app sigue. Enviado por un servidor SMTP tuyo — Gmail con una contraseña de aplicación funciona — tras la sincronización diaria, el día que elijas. Es lo único que esta app envía a algún sitio que no sea tu banco: a tu propio buzón. La contraseña se guarda junto a la clave bancaria.",
    "Port":
        "Puerto",
    "Preview today's mail":
        "Ver el correo de hoy",
    "Price move, 7 days":
        "Movimiento de precio, 7 días",
    "SMTP server":
        "Servidor SMTP",
    "Saturday":
        "Sábado",
    ...
    "Send the weekly e-mail":
        "Enviar el correo semanal",
    "Send this week's report now":
        "Enviar el informe de esta semana ahora",
    "Sent to {to}: {subject}":
        "Enviado a {to}: {subject}",
    "Sunday":
        "Domingo",
    "Test mail sent to {to}.":
        "Correo de prueba enviado a {to}.",
    "Test mail — the e-mail settings work.":
        "Correo de prueba — los ajustes de correo funcionan.",
    ...
    "Wealth Dashboard — test mail":
        "Wealth Dashboard — correo de prueba",
    "Wednesday":
        "Miércoles",
    "Weekly e-mail":
        "Correo semanal",
    "Weekly e-mail forgotten.":
        "Correo semanal olvidado.",
    "Weekly e-mail settings saved.":
        "Ajustes del correo semanal guardados.",
    "Weekly report":
        "Informe semanal",
    "Weekly report {date}":
        "Informe semanal {date}",
    "Weekly report — {since} to {date}":
        "Informe semanal — del {since} al {date}",
    "every {day}":
        "cada {day}",
    "optional; the user name otherwise":
        "opcional; si no, el nombre de usuario",
    "set up, switched off":
        "configurado, apagado",
    ...
    "Statement PDFs":
        "PDF de liquidación",
    'Banks & formats': 'Bancos y formatos',
    'Is your bank in there?': '¿Está tu banco?',
    'Every bank, broker and format the app reads, with a search box.': 'Todos los bancos, brókeres y formatos que lee la app, con buscador.',
    "What the import page can read, by name: the statement PDFs of banks and brokers, the CSV exports it knows outright, and the statement formats every bank writes alike. Drop the file on an account's import page — it is recognised by what is in it.": 'Lo que la página de importación puede leer, por nombre: los PDF de extractos de bancos y brókeres, las exportaciones CSV que conoce de por sí y los formatos de extracto que todos los bancos escriben igual. Suelta el archivo en la página de importación de una cuenta: se reconoce por su contenido.',
    'The Wertpapierabrechnungen, Dividendengutschriften and Kontoauszüge of some 150 banks and brokers are read from the PDFs in their mailbox, many at once or as a ZIP — see Banks & formats in the menu.': 'Las liquidaciones de valores, abonos de dividendos y extractos de unos 150 bancos y brókeres se leen de los PDF de su buzón, muchos a la vez o en un ZIP — ver Bancos y formatos en el menú.',
    'Your bank is not here?': '¿Tu banco no está?',
    'Every reader was written from one sample statement, and yours can be the next. Send a redacted statement — name, address, account number and IBAN painted over; dates, amounts, ISINs and every printed label kept — and it becomes a reader in the next release. A PDF with the boxes blacked out is fine, and so is its text layer copied out of a viewer. One document per kind helps most: a purchase, a dividend, a Kontoauszug.': 'Cada lector nació de un extracto de muestra, y el tuyo puede ser el siguiente. Envía un extracto anonimizado — nombre, dirección, número de cuenta e IBAN tapados; fechas, importes, ISIN y cada etiqueta impresa intactos — y se convierte en lector en la próxima versión. Vale un PDF con los campos tachados, y también su capa de texto copiada de un visor. Un documento por tipo ayuda más: una compra, un dividendo, un extracto.',
    'Send a statement sample': 'Enviar un extracto de muestra',
    'opens a GitHub issue with a short form and a place to attach the file': 'abre una incidencia en GitHub con un formulario breve y un sitio para adjuntar el archivo',
    'The same goes for a bank that is listed but reads a figure wrong: the sample that shows it is the fix.': 'Lo mismo vale para un banco listado que lee mal una cifra: la muestra que lo muestra es el arreglo.',
    '{n} banks, brokers and formats. Type a name to find yours; a bank not here is read through a CSV mapping you draw once — or send a statement, above.': '{n} bancos, brókeres y formatos. Escribe un nombre para encontrar el tuyo; un banco que no esté se lee con una asignación CSV que dibujas una vez — o envía un extracto, arriba.',
    'Nothing by that name — send a statement sample, above, or map a CSV of it once on the import page.': 'Nada con ese nombre — envía un extracto de muestra, arriba, o asigna un CSV suyo una vez en la página de importación.',
    'API secret': 'Secreto de la API',
    'Add your Interactive Brokers Flex token under Settings first.': 'Añade primero tu token Flex de Interactive Brokers en Ajustes.',
    'Add your Trading 212 API key under Settings first.': 'Añade primero tu clave API de Trading 212 en Ajustes.',
    'Approve the login in the Trade Republic app, then:': 'Aprueba el inicio de sesión en la app de Trade Republic y luego:',
    'Code from the Trade Republic app': 'Código de la app de Trade Republic',
    'Connect Interactive Brokers': 'Conectar Interactive Brokers',
    'Connect Trade Republic': 'Conectar Trade Republic',
    'Connect Trading 212': 'Conectar Trading 212',
    'Finish the login': 'Terminar el inicio de sesión',
    'Flex Query id': 'Id de la Flex Query',
    'Flex Web Service token': 'Token del Flex Web Service',
    'Forget Trade Republic': 'Olvidar Trade Republic',
    'Forget the Interactive Brokers token': 'Olvidar el token de Interactive Brokers',
    'Forget the Trading 212 key': 'Olvidar la clave de Trading 212',
    'In the Trading 212 app: Settings → API (Beta) → Generate key. Give it only the read scopes — account data, portfolio, history — and paste the key and the secret here; the secret is shown once. Invest and Stocks ISA accounts only. The API allows six history calls a minute, so a first sync of years of orders takes a few minutes; later syncs are quick.': 'En la app de Trading 212: Ajustes → API (Beta) → Generar clave. Dale solo los permisos de lectura — datos de cuenta, cartera, historial — y pega aquí la clave y el secreto; el secreto se muestra una vez. Solo cuentas Invest y Stocks ISA. La API permite seis llamadas de historial por minuto, así que una primera sincronización de años de órdenes tarda unos minutos; las siguientes son rápidas.',
    'Interactive Brokers answers: account {accounts}, statement {start} to {end}, {trades} trades. Now connect an account from its page.': 'Interactive Brokers responde: cuenta {accounts}, extracto de {start} a {end}, {trades} operaciones. Ahora conecta una cuenta desde su página.',
    'Interactive Brokers token forgotten. The account and its history stay.': 'Token de Interactive Brokers olvidado. La cuenta y su historial se quedan.',
    'Interactive Brokers: link this account to the Flex query under Settings and pull every trade, dividend and cash movement it covers.': 'Interactive Brokers: vincula esta cuenta a la Flex Query de Ajustes y trae cada operación, dividendo y movimiento de efectivo que cubre.',
    'Log in to Trade Republic': 'Iniciar sesión en Trade Republic',
    'Log in to Trade Republic first.': 'Inicia sesión primero en Trade Republic.',
    'Logged in to Trade Republic, securities account {account}.': 'Sesión iniciada en Trade Republic, cuenta de valores {account}.',
    "No login and no app: IBKR's Flex Web Service. In Account Management → Reports → Flex Queries, make an Activity Flex Query with Trades, Cash Transactions, Open Positions and Cash Report ticked, period Last 365 Days, XML; note its id. Under Reports → Settings → Flex Web Service, switch it on and copy the token. The token can only run Flex queries — it cannot see your login, trade or move money. For what lies before the year the query covers, run it once for a custom period and drop the XML on the import page.": 'Sin inicio de sesión ni app: el Flex Web Service de IBKR. En Account Management → Reports → Flex Queries crea una Activity Flex Query con Trades, Cash Transactions, Open Positions y Cash Report marcados, periodo Last 365 Days, XML; anota su id. En Reports → Settings → Flex Web Service actívalo y copia el token. El token solo puede ejecutar Flex Queries — no ve tu acceso, no opera ni mueve dinero. Para lo anterior al año que cubre la consulta, ejecútala una vez con un periodo personalizado y suelta el XML en la página de importación.',
    'Not approved yet — approve the login in the Trade Republic app, then press Finish again.': 'Aún no aprobado — aprueba el inicio de sesión en la app de Trade Republic y pulsa Terminar de nuevo.',
    'PIN': 'PIN',
    'Phone number': 'Número de teléfono',
    'Save and run the query once': 'Guardar y ejecutar la consulta una vez',
    'The Trade Republic login has lapsed — log in again to resume syncing.': 'La sesión de Trade Republic ha caducado — inicia sesión de nuevo para seguir sincronizando.',
    'Trade Republic forgotten — phone, PIN and login. The account and its history stay.': 'Trade Republic olvidado — teléfono, PIN y sesión. La cuenta y su historial se quedan.',
    'Trade Republic is asking: approve the login in the app, then press Finish.': 'Trade Republic pregunta: aprueba el inicio de sesión en la app y pulsa Terminar.',
    "Trade Republic publishes no API. This uses the interface its own web app uses — phone number, PIN, the app's approval, then the same WebSocket — and it may stop working any day, as it has before; the statement PDFs always read. The phone number and PIN are kept 0600 beside the other keys; a login lasts until Trade Republic ends it, and the account page says when it has to be renewed.": 'Trade Republic no publica ninguna API. Esto usa la interfaz de su propia app web — número de teléfono, PIN, aprobación en la app y luego el mismo WebSocket — y puede dejar de funcionar cualquier día, como ya ha pasado; los PDF de extractos siempre se leen. El teléfono y el PIN se guardan 0600 junto a las otras claves; una sesión dura hasta que Trade Republic la cierra, y la página de la cuenta dice cuándo hay que renovarla.',
    'Trade Republic wants the code from the app: type it and press Finish.': 'Trade Republic quiere el código de la app: escríbelo y pulsa Terminar.',
    'Trade Republic, through the interface its web app uses — unofficial, and it may stop working any day; the statement PDFs always read.': 'Trade Republic a través de la interfaz de su app web — no oficial, puede dejar de funcionar cualquier día; los PDF de extractos siempre se leen.',
    'Trading 212 key forgotten. The account and its history stay.': 'Clave de Trading 212 olvidada. La cuenta y su historial se quedan.',
    'Trading 212 key works: {cash} {currency} free cash, {invested} invested. Now connect an account from its page.': 'La clave de Trading 212 funciona: {cash} {currency} de efectivo libre, {invested} invertido. Ahora conecta una cuenta desde su página.',
    'Trading 212: link this account to the API key under Settings and pull every order, dividend and cash movement. A long history takes a few minutes — the API allows six calls a minute.': 'Trading 212: vincula esta cuenta a la clave API de Ajustes y trae cada orden, dividendo y movimiento de efectivo. Un historial largo tarda unos minutos — la API permite seis llamadas por minuto.',
    'Unofficial interface; the login is kept until Trade Republic ends it. Syncs with the daily sync.': 'Interfaz no oficial; la sesión se mantiene hasta que Trade Republic la cierra. Se sincroniza con la sincronización diaria.',
    'live': 'real',
    'logged in': 'sesión iniciada',
    'not logged in': 'sin sesión',
    'practice': 'práctica',
    'practice account': 'cuenta de práctica',
    'token saved': 'token guardado',
    'unofficial': 'no oficial',
    'Phone number and PIN': 'Número de teléfono y PIN',
    'saved': 'guardado',
    'Log in': 'Iniciar sesión',
    'Trade Republic will ask its app to approve, or for a code.': 'Trade Republic pedirá aprobación en su app o un código.',
    'After step 1.': 'Después del paso 1.',
    'After step 2.': 'Después del paso 2.',
    'Connect to an account': 'Conectar a una cuenta',
    'a new account called Trade Republic': 'una cuenta nueva llamada Trade Republic',
    'Connect and sync': 'Conectar y sincronizar',
    'Logged in to Trade Republic, securities account {account}. Now pick the account below to connect.': 'Sesión iniciada en Trade Republic, cuenta de valores {account}. Ahora elige abajo la cuenta a conectar.',
    'Saved. Now step 2: log in.': 'Guardado. Ahora el paso 2: iniciar sesión.',
    '1 day': '1 día',
    '1 week': '1 semana',
    '3 months': '3 meses',
    '3 years': '3 años',
    'Since start': 'Desde el inicio',
    'from {date}: time-weighted return, and the gain beyond the money moved in the meantime': 'desde {date}: rentabilidad ponderada por tiempo y la ganancia más allá del dinero movido entretanto',
    'nothing to measure yet': 'aún nada que medir',
    'Performance': 'Rendimiento',
    'Each period: the time-weighted return, and underneath the gain in money — what the value did beyond the deposits and withdrawals of the period, so a purchase is not a gain and a sale not a loss. A period the records do not reach back to is measured from the first day they do.': 'Por periodo: la rentabilidad ponderada por tiempo y debajo la ganancia en dinero — lo que hizo el valor más allá de los ingresos y retiros del periodo, así que una compra no es ganancia ni una venta pérdida. Un periodo al que los registros no llegan se mide desde su primer día.',
    'time-weighted and money-weighted, side by side': 'ponderada por tiempo y por dinero, lado a lado',
    'Portfolio performance': 'Rendimiento de la cartera',
    'the securities, time-weighted — cash left out': 'los valores, ponderados por tiempo — sin efectivo',
    '1 month': '1 mes',
    '1 year': '1 año',
    'A rule has to do something: file under a category, rename the counterparty, set the kind, add a tag, or say whose spending it is.': 'Una regla tiene que hacer algo: archivar en una categoría, renombrar la contraparte, fijar el tipo, añadir una etiqueta o decir de quién es el gasto.',
    'Doubled bank rows removed': 'Filas bancarias duplicadas eliminadas',
    'Its category_rules.json, if you have one': 'Su category_rules.json, si lo tienes',
    'No spending in this window.': 'Sin gastos en este periodo.',
    'Nobody yet': 'Nadie aún',
    'People created': 'Personas creadas',
    'Rules brought along': 'Reglas traídas',
    'Shared': 'Compartido',
    "The household's spending split between its people. A row of spending belongs to one of you, or to the household — shared, halved — or to nobody yet; what nobody has claimed sits in its own bucket rather than being quietly absorbed. Say whose a row is on the Transactions page, and it becomes a rule for the next one like it.": 'El gasto del hogar repartido entre sus personas. Un gasto pertenece a uno de vosotros, al hogar — compartido, a medias — o a nadie aún; lo que nadie ha reclamado queda en su propio cubo en vez de absorberse en silencio. Di de quién es una fila en la página Transacciones y se convierte en regla para la siguiente igual.',
    'The month before, {month}: {total} in all': 'El mes anterior, {month}: {total} en total',
    'Who spent': 'Quién gastó',
    'Whose spending': 'Gasto de quién',
    "each person's own spending plus half of what was shared": 'el gasto propio de cada persona más la mitad de lo compartido',
    'everything is assigned': 'todo está asignado',
    'last 12 months': 'últimos 12 meses',
    'month still running': 'mes en curso',
    'shared': 'compartido',
    'split evenly': 'a partes iguales',
    'whose spending': 'gasto de quién',
    ...
    'Forget the removed rows': 'Olvidar las filas eliminadas',
    ...
    'since the first day whose parts are known': 'desde el primer día cuyas partes se conocen',
    "Days recorded by another app are totals without parts, so with a switch off the line starts where this app's own daily figures do.": 'Los días registrados por otra app son totales sin partes, así que con un interruptor apagado la línea empieza donde empiezan las cifras diarias propias de esta app.',
    "An app on your phone asks for this dashboard's address and a six-digit code. The code is good for five minutes and for one device; it is exchanged once for the token above, over the same address you are reading this on. A token shown on a screen is a token anyone who photographs the screen owns, which is why the screen shows a code instead.": 'Una app en tu teléfono pide la dirección de este panel y un código de seis dígitos. El código vale cinco minutos y para un dispositivo; se cambia una vez por el token de arriba, por la misma dirección con la que estás leyendo esto. Un token mostrado en una pantalla es de quien fotografíe la pantalla: por eso se muestra un código.',
    "Choose ntfy instead and the body is a line of prose with a title and a priority — which is all a push server needs to put the message on a phone's lock screen. The URL is then your topic, say https://ntfy.sh/my-secret-topic or your own server's; anyone who knows the topic can read it, so make it long and private.": 'Si eliges ntfy, el cuerpo es una frase con título y prioridad — todo lo que un servidor de push necesita para poner el mensaje en la pantalla de bloqueo. La URL es entonces tu tema, por ejemplo https://ntfy.sh/mi-tema-secreto o el de tu propio servidor; quien conozca el tema puede leerlo, así que hazlo largo y privado.',
    'JSON, signed': 'JSON, firmado',
    'New code': 'Código nuevo',
    'Pair a device': 'Vincular un dispositivo',
    'Pair a phone or a tablet': 'Vincular un teléfono o una tableta',
    'Pairing code {code} — good for five minutes, and for one device.': 'Código de vinculación {code} — válido cinco minutos y para un dispositivo.',
    'Speaks': 'Habla',
    'The address to type into the app:': 'La dirección para escribir en la app:',
    'a code, not a token typed by thumb': 'un código, no un token escrito con el pulgar',
    '{n} second left': 'queda {n} segundo',
    '{n} seconds left': 'quedan {n} segundos',
    'a scan, with no text in it — nothing can be read from a picture of a page. Run it through OCR first (Paperless-ngx does this on its own), or install ocrmypdf on the server and upload it again.': 'un escaneo, sin texto dentro — de una imagen de una página no se puede leer nada. Pásalo primero por OCR (Paperless-ngx lo hace solo) o instala ocrmypdf en el servidor y vuelve a subirlo.',
    'A month': 'Al mes',
    'An average over months you have only half recorded is half an average; pick a window you trust.': 'Un promedio sobre meses registrados a medias es medio promedio; elige un periodo en el que confíes.',
    'Over': 'Sobre',
    'That is not a transaction.': 'Eso no es una transacción.',
    'The big ones': 'Los grandes',
    'a single payment can be left out of the monthly figures, or spread over the years it is used — the amount on the row does not change': 'un pago único puede dejarse fuera de las cifras mensuales o repartirse por los años de uso — el importe de la fila no cambia',
    'its own month': 'en su mes',
    'left out': 'dejado fuera',
    'over {n} month': 'sobre {n} mes',
    'over {n} months': 'sobre {n} meses',
    '{n} year': '{n} año',
    '{n} years': '{n} años',
    'Nothing matches those terms today. Check the spelling against the description as the Transactions page shows it, and remember that the direction, the amount range, the account and the kind all have to fit too.': 'Hoy nada coincide con esos términos. Comprueba la escritura contra la descripción tal como la muestra la página Transacciones, y recuerda que la dirección, el rango de importe, la cuenta y el tipo también deben encajar.',
    '{n} rows match, and every one of them is a purchase or a sale. A rule does not file trades unless it says which account or which kind it means — pick the account (or the kind) in the rule, and it will.': '{n} filas coinciden, y todas son una compra o una venta. Una regla no archiva operaciones a menos que diga qué cuenta o qué tipo quiere decir — elige la cuenta (o el tipo) en la regla y lo hará.',
    '{n} of the matching rows are purchases or sales and were left alone: a rule files trades only when it names an account or a kind.': '{n} de las filas coincidentes son compras o ventas y se dejaron intactas: una regla archiva operaciones solo cuando nombra una cuenta o un tipo.',
