...
STRINGS: dict[str, str] = {
    "Documents from Paperless": "Dokumente aus Paperless",
    "This account's documents": "Die Dokumente dieses Kontos",
    "See what matches": "Zeigen, was passt",
    "What this would pull": "Was damit geholt würde",
    "{n} document in the archive matches": "{n} Dokument im Archiv passt",
    "{n} documents in the archive match": "{n} Dokumente im Archiv passen",
    "Document": "Dokument",
    "Filed": "Abgelegt",
    "State": "Stand",
    "already read": "schon gelesen",
    "new": "neu",
    "The first {shown} of {matched} are shown.": "Gezeigt werden die ersten {shown} von {matched}.",
    "Nothing in the archive matches all of these at once.": "Nichts im Archiv erfüllt all das zugleich.",
    "Leave them all blank and this account pulls nothing.": "Bleibt alles leer, holt dieses Konto nichts.",
    "Which documents in the archive belong to {account}. Everything named has to hold at once — every tag, that correspondent, that type — so the pull can be surgical. The daily sync fetches whatever is new; nothing is read twice.": "Welche Dokumente im Archiv zu {account} gehören. Alles Genannte muss zugleich zutreffen — jeder Tag, dieser Korrespondent, dieser Typ —, damit gezielt geholt wird. Der tägliche Abgleich holt, was neu ist; nichts wird zweimal gelesen.",
    "Which Paperless documents are this account's — tags, correspondent, type — is set where it is used: {link}.": "Welche Paperless-Dokumente zu diesem Konto gehören — Tags, Korrespondent, Typ — wird dort eingestellt, wo es gebraucht wird: {link}.",
    "This account does not say which documents are its yet — name the tags, correspondent or type here.": "Dieses Konto sagt noch nicht, welche Dokumente seine sind — Tags, Korrespondent oder Typ hier eintragen.",
    ...
    "Rules": "Regeln",
    "Re-apply every rule": "Alle Regeln erneut anwenden",
    "Every rule re-applied: {n} transaction filed.": "Alle Regeln erneut angewendet: {n} Transaktion zugeordnet.",
    "Every rule re-applied: {n} transactions filed.": "Alle Regeln erneut angewendet: {n} Transaktionen zugeordnet.",
    "not a subscription": "kein Abo",
    "it is a subscription": "ist ein Abo",
    "put it back": "zurückholen",
    "Dismissed": "Verworfen",
    "you said these are not subscriptions; they stay out of the totals": "von Ihnen als kein Abo markiert; sie zählen nicht mit",
    ...
    'Statement formats': 'Kontoauszugsformate',
    "CAMT.053 / CAMT.052 XML (the ISO 20022 export of every European online-banking portal), MT940 (.sta, SWIFT) and OFX / QFX (North American and British banks and brokers) are read as they are — no mapping, the bank's own reference as the id, the closing balance taken.": 'CAMT.053 / CAMT.052 XML (der ISO-20022-Export jedes europäischen Online-Bankings), MT940 (.sta, SWIFT) und OFX / QFX (nordamerikanische und britische Banken und Broker) werden gelesen, wie sie sind — keine Zuordnung, die bankeigene Referenz als Id, der Schlusssaldo übernommen.',
    ...
    "Statement PDFs":
        "Abrechnungs-PDFs",
    'Banks & formats': 'Banken & Formate',
    'Is your bank in there?': 'Ist deine Bank dabei?',
    'Every bank, broker and format the app reads, with a search box.': 'Jede Bank, jeder Broker und jedes Format, das die App liest, mit Suchfeld.',
    "What the import page can read, by name: the statement PDFs of banks and brokers, the CSV exports it knows outright, and the statement formats every bank writes alike. Drop the file on an account's import page — it is recognised by what is in it.": 'Was die Importseite lesen kann, beim Namen: die Abrechnungs-PDFs von Banken und Brokern, die CSV-Exporte, die sie von sich aus kennt, und die Kontoauszugsformate, die jede Bank gleich schreibt. Leg die Datei auf der Importseite eines Kontos ab — sie wird an ihrem Inhalt erkannt.',
    'The Wertpapierabrechnungen, Dividendengutschriften and Kontoauszüge of some 150 banks and brokers are read from the PDFs in their mailbox, many at once or as a ZIP — see Banks & formats in the menu.': 'Die Wertpapierabrechnungen, Dividendengutschriften und Kontoauszüge von rund 150 Banken und Brokern werden aus den PDFs in deren Postfach gelesen, viele auf einmal oder als ZIP — siehe Banken & Formate im Menü.',
    'Your bank is not here?': 'Deine Bank ist nicht dabei?',
    'Every reader was written from one sample statement, and yours can be the next. Send a redacted statement — name, address, account number and IBAN painted over; dates, amounts, ISINs and every printed label kept — and it becomes a reader in the next release. A PDF with the boxes blacked out is fine, and so is its text layer copied out of a viewer. One document per kind helps most: a purchase, a dividend, a Kontoauszug.': 'Jeder Leser ist aus einer Musterabrechnung entstanden, und deine kann die nächste sein. Schick eine geschwärzte Abrechnung — Name, Adresse, Kontonummer und IBAN übermalt; Daten, Beträge, ISINs und jede gedruckte Beschriftung belassen — und sie wird im nächsten Release ein Leser. Ein PDF mit geschwärzten Feldern reicht, ebenso seine aus einem Viewer kopierte Textebene. Ein Dokument je Art hilft am meisten: ein Kauf, eine Dividende, ein Kontoauszug.',
    'Send a statement sample': 'Musterabrechnung senden',
    'opens a GitHub issue with a short form and a place to attach the file': 'öffnet ein GitHub-Issue mit kurzem Formular und Platz zum Anhängen der Datei',
    'The same goes for a bank that is listed but reads a figure wrong: the sample that shows it is the fix.': 'Dasselbe gilt für eine gelistete Bank, bei der eine Zahl falsch gelesen wird: das Muster, das es zeigt, ist die Korrektur.',
    '{n} banks, brokers and formats. Type a name to find yours; a bank not here is read through a CSV mapping you draw once — or send a statement, above.': '{n} Banken, Broker und Formate. Tipp einen Namen, um deine zu finden; eine Bank, die fehlt, wird über eine einmal gezeichnete CSV-Zuordnung gelesen — oder schick eine Abrechnung, oben.',
    'Nothing by that name — send a statement sample, above, or map a CSV of it once on the import page.': 'Nichts unter diesem Namen — schick eine Musterabrechnung, oben, oder ordne eine CSV davon einmal auf der Importseite zu.',
    'API secret': 'API-Secret',
    'Add your Interactive Brokers Flex token under Settings first.': 'Trag zuerst dein Interactive-Brokers-Flex-Token unter Einstellungen ein.',
    'Add your Trading 212 API key under Settings first.': 'Trag zuerst deinen Trading-212-API-Schlüssel unter Einstellungen ein.',
    'Approve the login in the Trade Republic app, then:': 'Bestätige die Anmeldung in der Trade-Republic-App, dann:',
    'Code from the Trade Republic app': 'Code aus der Trade-Republic-App',
    'Connect Interactive Brokers': 'Interactive Brokers verbinden',
    'Connect Trade Republic': 'Trade Republic verbinden',
    'Connect Trading 212': 'Trading 212 verbinden',
    'Finish the login': 'Anmeldung abschließen',
    'Flex Query id': 'Flex-Query-Id',
    'Flex Web Service token': 'Flex-Web-Service-Token',
    'Forget Trade Republic': 'Trade Republic vergessen',
    'Forget the Interactive Brokers token': 'Interactive-Brokers-Token vergessen',
    'Forget the Trading 212 key': 'Trading-212-Schlüssel vergessen',
    'In the Trading 212 app: Settings → API (Beta) → Generate key. Give it only the read scopes — account data, portfolio, history — and paste the key and the secret here; the secret is shown once. Invest and Stocks ISA accounts only. The API allows six history calls a minute, so a first sync of years of orders takes a few minutes; later syncs are quick.': 'In der Trading-212-App: Einstellungen → API (Beta) → Schlüssel erzeugen. Gib ihm nur die Lese-Scopes — Kontodaten, Portfolio, Historie — und füge Schlüssel und Secret hier ein; das Secret wird einmal gezeigt. Nur Invest- und Stocks-ISA-Konten. Die API erlaubt sechs Historien-Abrufe pro Minute, ein erster Sync über Jahre von Orders dauert also ein paar Minuten; spätere sind schnell.',
    'Interactive Brokers answers: account {accounts}, statement {start} to {end}, {trades} trades. Now connect an account from its page.': 'Interactive Brokers antwortet: Konto {accounts}, Auszug {start} bis {end}, {trades} Trades. Verbinde jetzt ein Konto von seiner Seite aus.',
    'Interactive Brokers token forgotten. The account and its history stay.': 'Interactive-Brokers-Token vergessen. Konto und Verlauf bleiben.',
    'Interactive Brokers: link this account to the Flex query under Settings and pull every trade, dividend and cash movement it covers.': 'Interactive Brokers: verknüpfe dieses Konto mit der Flex Query unter Einstellungen und hol jeden Trade, jede Dividende und jede Geldbewegung, die sie abdeckt.',
    'Log in to Trade Republic': 'Bei Trade Republic anmelden',
    'Log in to Trade Republic first.': 'Melde dich zuerst bei Trade Republic an.',
    'Logged in to Trade Republic, securities account {account}.': 'Bei Trade Republic angemeldet, Depot {account}.',
    "No login and no app: IBKR's Flex Web Service. In Account Management → Reports → Flex Queries, make an Activity Flex Query with Trades, Cash Transactions, Open Positions and Cash Report ticked, period Last 365 Days, XML; note its id. Under Reports → Settings → Flex Web Service, switch it on and copy the token. The token can only run Flex queries — it cannot see your login, trade or move money. For what lies before the year the query covers, run it once for a custom period and drop the XML on the import page.": 'Kein Login und keine App: IBKRs Flex Web Service. Leg unter Account Management → Reports → Flex Queries eine Activity Flex Query an mit Trades, Cash Transactions, Open Positions und Cash Report angehakt, Zeitraum Last 365 Days, XML; merk dir ihre Id. Unter Reports → Settings → Flex Web Service schaltest du den Dienst ein und kopierst das Token. Das Token kann nur Flex Queries ausführen — es sieht dein Login nicht, handelt nicht und bewegt kein Geld. Für alles vor dem Jahr, das die Query abdeckt, führ sie einmal mit eigenem Zeitraum aus und leg das XML auf der Importseite ab.',
    'Not approved yet — approve the login in the Trade Republic app, then press Finish again.': 'Noch nicht bestätigt — bestätige die Anmeldung in der Trade-Republic-App und drück dann wieder Abschließen.',
    'PIN': 'PIN',
    'Phone number': 'Telefonnummer',
    'Save and run the query once': 'Speichern und die Query einmal ausführen',
    'The Trade Republic login has lapsed — log in again to resume syncing.': 'Die Trade-Republic-Anmeldung ist abgelaufen — melde dich erneut an, damit der Sync weitergeht.',
    'Trade Republic forgotten — phone, PIN and login. The account and its history stay.': 'Trade Republic vergessen — Telefon, PIN und Anmeldung. Konto und Verlauf bleiben.',
    'Trade Republic is asking: approve the login in the app, then press Finish.': 'Trade Republic fragt: bestätige die Anmeldung in der App und drück dann Abschließen.',
    "Trade Republic publishes no API. This uses the interface its own web app uses — phone number, PIN, the app's approval, then the same WebSocket — and it may stop working any day, as it has before; the statement PDFs always read. The phone number and PIN are kept 0600 beside the other keys; a login lasts until Trade Republic ends it, and the account page says when it has to be renewed.": 'Trade Republic veröffentlicht keine API. Dies nutzt die Schnittstelle der eigenen Web-App — Telefonnummer, PIN, Bestätigung in der App, dann derselbe WebSocket — und kann jeden Tag aufhören zu funktionieren, wie schon zuvor; die Abrechnungs-PDFs lesen immer. Telefonnummer und PIN liegen 0600 neben den anderen Schlüsseln; eine Anmeldung hält, bis Trade Republic sie beendet, und die Kontoseite sagt, wann sie erneuert werden muss.',
    'Trade Republic wants the code from the app: type it and press Finish.': 'Trade Republic will den Code aus der App: tipp ihn ein und drück Abschließen.',
    'Trade Republic, through the interface its web app uses — unofficial, and it may stop working any day; the statement PDFs always read.': 'Trade Republic über die Schnittstelle seiner Web-App — inoffiziell, kann jeden Tag aufhören zu funktionieren; die Abrechnungs-PDFs lesen immer.',
    'Trading 212 key forgotten. The account and its history stay.': 'Trading-212-Schlüssel vergessen. Konto und Verlauf bleiben.',
    'Trading 212 key works: {cash} {currency} free cash, {invested} invested. Now connect an account from its page.': 'Trading-212-Schlüssel funktioniert: {cash} {currency} freies Guthaben, {invested} investiert. Verbinde jetzt ein Konto von seiner Seite aus.',
    'Trading 212: link this account to the API key under Settings and pull every order, dividend and cash movement. A long history takes a few minutes — the API allows six calls a minute.': 'Trading 212: verknüpfe dieses Konto mit dem API-Schlüssel unter Einstellungen und hol jede Order, jede Dividende und jede Geldbewegung. Ein langer Verlauf dauert ein paar Minuten — die API erlaubt sechs Abrufe pro Minute.',
    'Unofficial interface; the login is kept until Trade Republic ends it. Syncs with the daily sync.': 'Inoffizielle Schnittstelle; die Anmeldung bleibt, bis Trade Republic sie beendet. Läuft mit dem täglichen Sync.',
    'live': 'live',
    'logged in': 'angemeldet',
    'not logged in': 'nicht angemeldet',
    'practice': 'Übungskonto',
    'practice account': 'Übungskonto',
    'token saved': 'Token gespeichert',
    'unofficial': 'inoffiziell',
    'Phone number and PIN': 'Telefonnummer und PIN',
    'saved': 'gespeichert',
    'Log in': 'Anmelden',
    'Trade Republic will ask its app to approve, or for a code.': 'Trade Republic lässt seine App bestätigen oder fragt nach einem Code.',
    'After step 1.': 'Nach Schritt 1.',
    'After step 2.': 'Nach Schritt 2.',
    'Connect to an account': 'Mit einem Konto verbinden',
    'a new account called Trade Republic': 'ein neues Konto namens Trade Republic',
    'Connect and sync': 'Verbinden und synchronisieren',
    'Logged in to Trade Republic, securities account {account}. Now pick the account below to connect.': 'Bei Trade Republic angemeldet, Depot {account}. Wähle jetzt unten das Konto zum Verbinden.',
    'Saved. Now step 2: log in.': 'Gespeichert. Jetzt Schritt 2: anmelden.',
    '1 day': '1 Tag',
    '1 week': '1 Woche',
    '3 months': '3 Monate',
    '3 years': '3 Jahre',
    'Since start': 'Seit Beginn',
    'from {date}: time-weighted return, and the gain beyond the money moved in the meantime': 'ab {date}: zeitgewichtete Rendite und der Gewinn über das inzwischen bewegte Geld hinaus',
    'nothing to measure yet': 'noch nichts zu messen',
    'Performance': 'Performance',
    'Each period: the time-weighted return, and underneath the gain in money — what the value did beyond the deposits and withdrawals of the period, so a purchase is not a gain and a sale not a loss. A period the records do not reach back to is measured from the first day they do.': 'Je Zeitraum: die zeitgewichtete Rendite und darunter der Gewinn in Geld — was der Wert über die Ein- und Auszahlungen des Zeitraums hinaus getan hat, sodass ein Kauf kein Gewinn und ein Verkauf kein Verlust ist. Ein Zeitraum, so weit die Aufzeichnungen nicht zurückreichen, wird ab ihrem ersten Tag gemessen.',
    'time-weighted and money-weighted, side by side': 'zeit- und geldgewichtet, nebeneinander',
    'Portfolio performance': 'Portfolio-Performance',
    'the securities, time-weighted — cash left out': 'die Wertpapiere, zeitgewichtet — ohne Cash',
    '1 month': '1 Monat',
    '1 year': '1 Jahr',
    'A rule has to do something: file under a category, rename the counterparty, set the kind, add a tag, or say whose spending it is.': 'Eine Regel muss etwas tun: unter einer Kategorie ablegen, den Gegenpart umbenennen, die Art setzen, ein Tag anhängen oder sagen, wessen Ausgabe es ist.',
    'Doubled bank rows removed': 'Doppelte Bankzeilen entfernt',
    'Its category_rules.json, if you have one': 'Seine category_rules.json, falls vorhanden',
    'No spending in this window.': 'Keine Ausgaben in diesem Zeitraum.',
    'Nobody yet': 'Noch niemand',
    'People created': 'Personen angelegt',
    'Rules brought along': 'Mitgebrachte Regeln',
    'Shared': 'Gemeinsam',
    "The household's spending split between its people. A row of spending belongs to one of you, or to the household — shared, halved — or to nobody yet; what nobody has claimed sits in its own bucket rather than being quietly absorbed. Say whose a row is on the Transactions page, and it becomes a rule for the next one like it.": 'Die Ausgaben des Haushalts, aufgeteilt auf seine Personen. Eine Ausgabe gehört einem von euch, dem Haushalt — gemeinsam, halbiert — oder noch niemandem; was niemand beansprucht hat, steht in einem eigenen Topf, statt still aufgesogen zu werden. Sag auf der Seite Transaktionen, wessen eine Zeile ist, und daraus wird eine Regel für die nächste ihrer Art.',
    'The month before, {month}: {total} in all': 'Der Monat davor, {month}: {total} insgesamt',
    'Who spent': 'Wer hat ausgegeben',
    'Whose spending': 'Wessen Ausgabe',
    "each person's own spending plus half of what was shared": 'die eigenen Ausgaben jeder Person plus die Hälfte des Gemeinsamen',
    'everything is assigned': 'alles ist zugeordnet',
    'last 12 months': 'letzte 12 Monate',
    'month still running': 'Monat läuft noch',
    'shared': 'gemeinsam',
    'split evenly': 'gleich geteilt',
    'whose spending': 'wessen Ausgabe',
    ...
    'Forget the removed rows': 'Die entfernten Zeilen vergessen',
    ...
    "An app on your phone asks for this dashboard's address and a six-digit code. The code is good for five minutes and for one device; it is exchanged once for the token above, over the same address you are reading this on. A token shown on a screen is a token anyone who photographs the screen owns, which is why the screen shows a code instead.": 'Eine App auf deinem Telefon fragt nach der Adresse dieses Dashboards und nach einem sechsstelligen Code. Der Code gilt fünf Minuten und für ein Gerät; er wird einmal gegen das Token oben getauscht, über dieselbe Adresse, unter der du das hier liest. Ein Token, das auf einem Bildschirm steht, gehört jedem, der den Bildschirm fotografiert — darum steht dort ein Code.',
    "Choose ntfy instead and the body is a line of prose with a title and a priority — which is all a push server needs to put the message on a phone's lock screen. The URL is then your topic, say https://ntfy.sh/my-secret-topic or your own server's; anyone who knows the topic can read it, so make it long and private.": 'Wählst du stattdessen ntfy, ist der Rumpf ein Satz mit Titel und Priorität — mehr braucht ein Push-Server nicht, um die Meldung auf den Sperrbildschirm eines Telefons zu bringen. Die URL ist dann dein Topic, etwa https://ntfy.sh/mein-geheimes-topic oder das deines eigenen Servers; wer das Topic kennt, liest mit, also mach es lang und privat.',
    'JSON, signed': 'JSON, signiert',
    'New code': 'Neuer Code',
    'Pair a device': 'Gerät koppeln',
    'Pair a phone or a tablet': 'Telefon oder Tablet koppeln',
    'Pairing code {code} — good for five minutes, and for one device.': 'Kopplungscode {code} — gültig für fünf Minuten und für ein Gerät.',
    'Speaks': 'Spricht',
    'The address to type into the app:': 'Die Adresse für die App:',
    'a code, not a token typed by thumb': 'ein Code, kein mit dem Daumen getipptes Token',
    '{n} second left': 'noch {n} Sekunde',
    '{n} seconds left': 'noch {n} Sekunden',
    'a scan, with no text in it — nothing can be read from a picture of a page. Run it through OCR first (Paperless-ngx does this on its own), or install ocrmypdf on the server and upload it again.': 'ein Scan, ohne Text darin — aus einem Bild einer Seite lässt sich nichts lesen. Lass ihn zuerst durch eine OCR laufen (Paperless-ngx macht das von selbst) oder installiere ocrmypdf auf dem Server und lade ihn erneut hoch.',
    'A month': 'Pro Monat',
    'An average over months you have only half recorded is half an average; pick a window you trust.': 'Ein Schnitt über Monate, die nur halb erfasst sind, ist ein halber Schnitt; nimm einen Zeitraum, dem du traust.',
    'Over': 'Über',
    'That is not a transaction.': 'Das ist keine Buchung.',
    'The big ones': 'Die großen Posten',
    'a single payment can be left out of the monthly figures, or spread over the years it is used — the amount on the row does not change': 'eine einzelne Zahlung kann aus den Monatszahlen herausgelassen oder über die Jahre ihrer Nutzung verteilt werden — der Betrag der Zeile ändert sich nicht',
    'its own month': 'in seinem Monat',
    'left out': 'herausgelassen',
    'over {n} month': 'über {n} Monat',
